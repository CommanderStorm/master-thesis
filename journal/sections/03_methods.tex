...
Let $\mathcal{S}$ be the set of syntactically valid MapLibre style JavaScript Object Notation (JSON) documents, let $\mathcal{T}$ be the set of tile sets, and let $\mathcal{V}$ be the set of viewing states: the six camera degrees of freedom plus the viewport.
...
We therefore characterize each pass empirically in an ablation study whose per-axis breakdown lets an operator compose the pass set suiting the deployment priorities.
...
The empirical oracle is deliberately weaker than bit-identity: structural changes such as re-batched draw calls and re-encoded tile coordinates perturb edge antialiasing and floating-point rasterization without altering what is drawn.
...
A code-generation front end produces Rust types from the upstream MapLibre style specification (the published \texttt{v8.json}), shifting specification drift from manual synchronization to a regenerator invocation and giving every pass typed access to defaults, value types, and expression capabilities.
...
Each tile is handed over as a batch of parallel memory segments rather than one call per feature, amortizing the per-boundary cost over thousands of features per encoder invocation, the boundary shape a columnar encoder requires.
...
\centering
\small
\begin{tabularx}{\textwidth}{p{3.2cm} p{1.7cm} X}
\toprule
\textbf{Pass} & \textbf{Data} & \textbf{Description} \\
\midrule
...
Layer merging & none & Merge identical adjacent layers via synthesized \mlop{case}/\mlop{match} \\
...
Layer merging combines adjacent layers of the same type whose differing properties are all mergeable, synthesizing a \mlop{case} or \mlop{match} expression over the filter discriminator so fewer layers mean fewer render passes.
...
This makes shaving a projection-pushdown plan over the tile data, advised by the optimized style rather than the original.
...
The optimizer rewrites the style expressions to the integer indices, keeping the optimized style consistent with the transformed tiles.
...
Rather than committing to a single encoding plan, it uses each tile's data distribution to try multiple strategies, measure each output size, and select the smallest.
...
We deliberately made the inner criterion single-objective, smallest encoded size, and report the multi-axis deployment trade-offs separately in the ablation rather than resolving them inside the encoder.
...
Together these discharge the idempotency and visual-equivalence guarantees on the tile half.

A tile-statistics collection phase scans the tile set to produce a per-source-layer profile of feature counts, per-zoom counts, geometry-type counts, feature-identifier presence, and per-property statistics.
Integer and string properties additionally carry a full value-frequency map below a cardinality of 200.
A sample rate of 1.0 (full census) is required for irreversible decisions such as dead layer elimination or property interning, and a sample-rate guard enforces this before any such pass applies.
All of this runs offline, paid once per (style, tileset) build, and the total preprocessing cost decomposes into four additive terms covering style passes, statistics, advisory computation, and re-encoding, reported empirically.
