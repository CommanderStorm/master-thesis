...
Constant folding performs the role of constant propagation and dead-code elimination, evaluating boolean algebra over \texttt{all} and \texttt{any} nodes, comparisons between literals, and dead-branch elimination on \texttt{case} and \texttt{match}.
...
Contradiction detection rewrites mutually exclusive sibling predicates to \texttt{false}, which structural dead elimination later exploits to remove the enclosing layer.
When tile statistics are available, nine further rules fold expressions that are statically indeterminate but data-determinate, covering property presence, single values, geometry type, numeric ranges, \texttt{match}/\texttt{in} arm pruning and frequency reordering, and ramp-stop pruning outside the observed range.
...
For \texttt{all} it sorts predicates by ascending selectivity so the predicate most likely to be false is evaluated first, and for \texttt{any} it sorts by descending selectivity.
...
Dead layer elimination treats a layer as dead when its filter is literally \texttt{false}, when its source layer carries zero features, or when its geometry type does not match the available types, which is particularly effective on OMT, where one source layer carries mixed geometry.
Metadata refinement extracts implicit zoom bounds from filter predicates and paint ramps and lifts them into explicit \texttt{minzoom}/\texttt{maxzoom} metadata, allowing the renderer to skip whole layers at zoom levels where they are invisible.
Refinement is asymmetric to respect overzoom: \texttt{minzoom} may be raised to the first data-carrying zoom freely, but \texttt{maxzoom} is tightened only when the last data-carrying zoom is strictly below the source's \texttt{maxzoom}, since otherwise the layer's tiles would be overzoomed beyond that level.
...
Layer merging combines adjacent layers of the same type whose differing properties are all mergeable, synthesizing a \texttt{case} or \texttt{match} expression over the filter discriminator so that fewer layers mean fewer render passes.
...
Three adjacent line layers differing only in filter literal, color, and width are merged into one whose paint properties and sort key become \mlop{match} expressions keyed on \mlval{"class"}.
...
\emph{Property usage} records each property name appearing in a \texttt{get} or \texttt{has} expression in any live layer, together with the zoom range over which it is referenced.
...
\emph{Categorical-value usage} records property values referenced by equality filters and \texttt{match} expressions, with their applicable zoom ranges.
...
The exact sketch catches columns sharing literal values (\texttt{class} and \texttt{subclass} in OMT) and the trigram sketch catches cross-language morphological variants (\texttt{name:de} and \texttt{name:fr}), both of which prefix matching misses.
