...
The encoder re-encodes the pruned and transformed data into the MLT columnar format.
...
Geometry vertices, roughly 70\% of MVT bytes on a planet OMT run, use a dedicated path competing componentwise deltas against Morton- and Hilbert-code dictionaries.
...
These catch columns sharing literal values (\mlval{class} and \mlval{subclass} in OMT) and cross-language variants (\mlval{name:de} and \mlval{name:fr}) that prefix matching misses.
