...
\begin{itemize}
    ...
    The expression level iterates peephole rewrites to a fixpoint.
    The structural level runs passes over a style model, namely dead layer elimination, metadata refinement, and layer merging.
    The tile level adds two techniques whose combination is to our knowledge novel.
    \emph{Style-driven tile shaving} derives a pruning advisory from the optimized style and removes unused properties, geometry types, values, and feature identifiers.
    \emph{Adaptive multi-strategy encoding} competes sort orderings, integer encodings, and string compression at encode time for the \ac{MLT} columnar format.
