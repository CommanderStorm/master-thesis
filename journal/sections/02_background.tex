...
\citet{tremmelMapLibreTileNext2025b} introduced \ac{MLT}, a columnar alternative that stores each property as an independent column with type-specific encoding, which enables per-column compression and removes the per-feature tag overhead.
...
A style declares \emph{sources} (WHAT to show), which name where tile data comes from, the source-layers that are avaliable within a source and metadata such as which zoom levels it is available at.
