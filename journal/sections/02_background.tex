...
When a request exceeds the source's maximum zoom, the renderer \emph{overzooms}, upscaling the nearest available tile.
A layer's effective visibility therefore depends on both its filter and the source's \mlprop{maxzoom}, not on layer bounds alone, which makes \mlprop{maxzoom} tightening riskier than \mlprop{minzoom} tightening.
...
The dominant wire format is Mapbox Vector Tiles (MVT), a Protocol Buffers encoding with a \emph{row-oriented} layout in which each feature stores its own property tags.
This layout means accessing a single property column requires parsing past all other properties, and compression operates on an interleaved byte stream rather than homogeneous columns.
\citet{tremmelMapLibreTileNext2025b} introduced MapLibre Tiles (MLT), a columnar alternative that stores each property as an independent column with type-specific encoding.
The columnar layout enables per-column compression and removes the per-feature tag overhead, reported as up to $3\times$ tile size reduction on encoded tilesets and up to $6\times$ on certain large tiles.
These are peak ratios measured against uncompressed MVT, and general-purpose compression closes most of the gap: as we show in Section~\ref{sec:results}, gzip-compressed MVT is already competitive with a fixed-plan columnar encoder on the average tile, so we benchmark mean size against gzipped MVT rather than the peak figures.
...
Two properties anchor the methods.
\emph{Visual equivalence} requires that an optimized style and tileset $(S', T')$ produce pixel-equivalent renderings to the original $(S, T)$ across the supported zoom and viewport range, which is what licenses any rewrite.
\emph{Idempotency} requires that re-applying an optimization pass to its own output yields no further change, which is the practical signature of having reached a normal form.
Both depend on respecting runtime state that lives outside the style and tile data: MapLibre carries per-feature \emph{feature state} and map-wide \emph{global state} stores, mutated by the host application at render time, so expressions reading them cannot be folded to constants.
