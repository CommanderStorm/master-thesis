...
Measured over a corpus of production and institutional styles under interactive navigation scenarios, discarding tile content the style never consumes accounts for most of the benefit, lowering median load time by 71\% and raising sustained frame rate to a median of \num{2.5}$\times$.
The columnar encoder alone shrinks tile volume by 28\% against the uncompressed reference and 12\% against gzip-compressed Mapbox Vector Tiles, and style rewriting shrinks unseen styles by a median of 30.8\% with visually equivalent output.
Quantifying how the two levels interact, we find their combined effect is additive rather than synergistic, with per-scenario synergy present but too sparse to raise the aggregate.
\end{abstract}
