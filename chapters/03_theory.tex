...
\begin{description}
\item[\ac{SVG}] \begin{sloppypar} is a vector graphics format that can be used to render maps as shown by the ID Editor\footnote{\url{https://github.com/openstreetmap/iD}, last accessed 2026-03-16} or Leaflet\footnote{\url{https://leafletjs.com/}, last accessed 2026-03-16}.
Using \acp{SVG} means one is limited to a 2D rendering approach due to not having a camera.\end{sloppypar}

\item[Canvas 2D] is the HTML5 \texttt{<canvas>} immediate-mode drawing \ac{API}.
OpenLayers\footnote{\url{https://openlayers.org/}, last accessed 2026-04-21} uses Canvas~2D as its primary renderer, rasterizing vector features on the \ac{CPU} rather than submitting them to the \ac{GPU}.
...
These renderers require data to be preprocessed and optimized specifically for rendering.
...
Data is taken from a data source, converted into a simplified format (e.g., a spatial index), and served to a client.
The visual appearance is defined by a style and supported by fonts and sprites for text and icons.}
	\label{fig:basic-arch}
\end{figure}

As shown in \cref{fig:basic-arch}, displaying map content at this lower level requires
    \emph{styles} that define the visual appearance and reference the other resources,
    \emph{tiles} that provide the geometry (points, lines, polygons) and associated attributes (e.g., building heights),
    \emph{sprites} that supply the icons, and
    \emph{fonts} that provide the glyphs used to render text labels.
...
In some cases, parts of these resources may be generated dynamically, e.g., to support complex text shaping for Arabic, Hebrew, Indic, or Japanese scripts as described by Wipfli~\cite{wipfliComplexTextPlugin2025} for MapLibre or Lengyel~\cite{lengelDecadeSlug} for SLUG.
...
\begin{description}
\item[Dynamic]
    Store geospatial data in a database (e.g., PostGIS\footnote{\url{https://postgis.net/}, last accessed 2026-05-23}) and use a tile server to generate and serve tiles on demand.
\item[Client-driven]
    Store encoded tiles as a single file in a blob store using a format that includes an internal tile index (e.g., PMTiles or VersaTiles).
    Clients retrieve tiles using multiple \ac{HTTP} requests with appropriate Range headers to locate the required data.
\item[Semi-static]
    Store encoded tiles in a database file on disk (e.g., SQLite) and use a tile server to extract and serve tiles to clients.
\end{description}
...
All of these resources can be optimized, but tiles are usually the most critical because they stream continuously as a user moves the map.
This typically gives them a cumulatively higher data volume, even though their individual tiles may be much smaller.
All other resources may depend on locale slightly (e.g., loading different Unicode ranges or fonts), but are only handled one time.

Real-world observations support this.
The performance of highly optimized systems such as Microsoft Bing Maps differs significantly from less optimized deployments.
...
This requires including a text shaping engine to the client, as neither native nor web targets have e.g., Harfbuzz\footnote{\url{https://github.com/harfbuzz/harfbuzz}, last accessed 2026-05-26} exposed as a feature.
...
A simpler approach therefore exists.
Fonts are cut into Unicode slices (\texttt{start..end}) and only the ranges that the client needs are served.
...
Common open-source renderers like MapLibre or source-available renderers like Mapbox do this by shipping pre-rasterized glyph ranges as \acp{PBF} encoded \acp{SDF}~\cite{greenImprovedAlphatestedMagnification2007}, indexed by Unicode range (e.g.,\ \texttt{0-255.pbf}, \texttt{256-511.pbf}).
...
\Cref{lst:overzoom} shows the two operations the slicer performs to synthesize a target tile from the source's \mlprop{maxzoom} tile.
...
Each tile is organized into \emph{source layers} (e.g., roads, buildings, water), and each source layer contains a set of \emph{features}.
A feature consists of geometry (point, linestring, or polygon encoded as integer coordinates relative to the tile's extent), optionally a set of key-value \emph{properties} (e.g., \mlval{class}=\mlval{"motorway"} or \mlval{name}=\mlval{"Main~Street"}), optionally a set of IDs, and metadata such as the tile extent.

The following formats are used for encoding and transferring data to the client:

\begin{description}
    \item[Raster tiles] deliver pre-rendered images (\ac{PNG}, JPEG, WebP, Avif, JPEGXL) rather than vector geometry.
    ...
    \item[Analysis-focused formats] such as FlatGeobuf\footnote{\url{http://flatgeobuf.org/}, last accessed 2026-05-11}, GeoParquet\footnote{\url{https://geoparquet.org/}, last accessed 2026-05-11}, and GeoArrow\footnote{\url{https://geoarrow.org/}, last accessed 2026-05-11} are optimized for analytical queries (filtering, aggregation, joins) rather than tile-based rendering.
    ...
    \item[\ac{GeoJSON}~\cite{butlerGeoJSONFormat2016}] is a \ac{JSON}-based geospatial interchange format.
    ...
    \item[\acf{MVT}]\footnote{\url{https://github.com/mapbox/vector-tile-spec/tree/master/2.1}, last accessed 2026-04-21} is the dominant vector tile format, adopted as a \emph{de~facto} standard across open-source map renderers including MapLibre.
...
\end{itemize}
    ...
    \item[\acf{MLT}] \begin{sloppypar} is a columnar vector tile format proposed by Tremmel et~al.~\cite{tremmelMapLibreTileNext2025b}.
    ...
    This columnar layout enables per-column compression strategies (e.g., dictionary encoding for low-cardinality string columns, delta encoding for monotonically increasing integers, or run-length encoding for repeated values) and eliminates the per-feature tag overhead of the protobuf encoding.
    Tremmel et~al.\ report up to $3\times$ tile size reduction on encoded tilesets and up to $6\times$ on certain large tiles, with the realized ratio depending on data characteristics.\end{sloppypar}
\end{description}

Beyond the choice of tile format, a number of transfer-layer optimizations can improve delivery performance:
utilizing the approximately \qty{14}{\kilo\byte} TCP slow-start window as discussed by Nathaniel~\cite{nathanielWhyYourWebsite2022}, edge caching\footnote{such as suggested by \url{https://docs.here.com/map-rendering/docs/cache-maps-raster-tile-api}, last accessed 2026-03-16}, HTTP/2 multiplexing~\cite{belsheHypertextTransferProtocol2015} (and its QUIC-based successor HTTP/3~\cite{bishopHTTP32022} for lossy mobile networks), and modern content-encoding algorithms such as Zstandard~\cite{colletZstandardCompressionApplication2021} and brotli~\cite{alakuijalaBrotliCompressedData2016} that achieve higher compression ratios than gzip's LZ77 foundation~\cite{zivUniversalAlgorithmSequential1977}.
...
A map style defines the complete visual appearance of a map, including which data is shown, how it is colored, sized, and labeled, and at which zoom levels each element appears.
Three architectural approaches to map styling have been established:

\begin{description}
    \item[Server-side declarative] styles, such as MapCSS, define rendering rules on the server.
    ...
    \item[Client-side declarative] \begin{sloppypar} styles, as used by MapLibre\footnote{\url{https://maplibre.org/}, last accessed 2026-03-16} and Mapbox\footnote{\url{https://www.mapbox.com/}, last accessed 2026-03-16}, transmit a machine-readable style document to the client, which interprets the style at render time.
    ...
    \item[Callback-based] styling, as proposed by Gleo\footnote{\url{https://ivansanchez.gitlab.io/gleo/}, last accessed 2026-03-16} or Navara\footnote{\url{https://navara-docs.netlify.app/}, last accessed 2026-04-21}, replace declarative rules with imperative callbacks that the application registers with the renderer.
...
It is widely adopted in the open-source map rendering ecosystem, and its declarative nature makes it amenable to static analysis and automatic optimizations.
...
\begin{description}
    \item[Sources] declare where tile data comes from (tile server URL, or inline \ac{GeoJSON}) and at which zoom levels tiles are available (\mlprop{minzoom}, \mlprop{maxzoom}).

    \item[Layers] are the central rendering unit.
    Each layer references a source and a \emph{source-layer} within that source, specifies a \emph{type} (\mlval{fill}, \mlval{line}, \mlval{circle}, \mlval{symbol}, \mlval{fill-extrusion}, \mlval{raster}, \mlval{background} or \mlval{hillshade}) and carries a set of paint and layout properties that control its appearance.
    Layers are drawn in declaration order from bottom to top.
    \begin{description}

    \item[Filters] are boolean expressions attached to layers that select which features from the source-layer are rendered.
    ...
    \item[Paint and layout properties] control the visual appearance and placement of rendered features.
    ...
    \item[Metadata bounds] (\mlprop{minzoom}, \mlprop{maxzoom}) restrict the zoom range in which a layer is active.
...
The worker calls \texttt{layer.isHidden(this.zoom, true)} and \texttt{continue}s before bucket creation, and the painter's \texttt{renderLayer} returns immediately on\newline
\texttt{layer.isHidden(this.transform.zoom)} before dispatching any \texttt{draw.fill}/ \texttt{draw.line}/ \texttt{draw.symbol} call.
\end{description}
\end{description}
...
Optimizing these expressions draws directly on techniques developed over decades in compiler construction~\cite{ahoCompilersPrinciplesTechniques2007}.
In this section we introduce the foundational concepts we apply in \cref{ch:methods}.

\subsection{Peephole Optimization}

\begin{sloppypar}
A \emph{peephole optimizer} examines a small, fixed-sized window of code (the \enquote{peephole}) and applies local pattern-matching rewrites that replace instruction sequences with shorter or faster equivalents~\cite{davidsonCodeSelectionObject1984}.
...
Compound predicates $p_i$ are estimated by combining individual selectivities:
...
In \ac{OMT}, for example, \mlval{class} and \mlval{subclass} are strongly correlated by schema design (every value of \mlval{subclass} implies a constrained set of \mlval{class} values), as are \mlval{admin\_level} and \mlval{maritime}.
...
This is distinct from profile-guided optimization, which would require measuring per-expression hotness at render time and specializing the hot paths.
The style optimizer never observes render-time execution and therefore does not perform that kind of specialization.
...
We characterize this as \emph{exhaustive strategy selection} over a bounded candidate space, optimal within that space but blind to strategies outside of it.
...
A column applies zero or more logical techniques in chain and then exactly one physical technique, e.g., DeltaRLE~$\rightarrow$~FastPFOR.

\Cref{sub:logical_integer_encodings,sub:physical_integer_compressions} catalog the techniques we use in our data-level optimizations of \cref{ch:methods}.
...
This layout is efficient for transactional workloads that access entire records, but wasteful for analytical queries that access only a few columns, as the entire record must be read to access every single attribute.
...
The foundational system in this space is by Dremel~\cite{melnikDremelInteractiveAnalysis2010}, who introduced the record shredding and assembly algorithm for nested columnar data using definition and repetition levels, the representation pattern that directly inspired Apache Parquet and, by extension, the columnar layout that \ac{MLT} adapts for tile data.
This layout offers three advantages for read-heavy workloads. \emph{Reduced I/O} follows from reading or transferring only the referenced columns. \emph{Better compression} follows from the higher local homogeneity of values of the same type and semantic domain compared to interleaved multi-type records. \emph{Vectorized processing} follows from the layout's natural fit for \ac{SIMD}-style batch operations over arrays of the same type.
...
A column of \mlval{road} classes can use dictionary encoding, while a column of \mlval{building\_height} can use a DeltaRLE logical chain handed to a physical technique such as FastPFOR, each chosen independently.
...
\begin{description}
    \item[Zigzag encoding] maps signed integers to unsigned ones ($n \mapsto 2n$ for $n \geq 0$, $n \mapsto -2n-1$ for $n < 0$) so that small-magnitude negative values also produce small unsigned codes.
    The byte count is unchanged on its own and the saving comes from the downstream physical technique.

    \item[Delta encoding] replaces each value with the difference from its predecessor.
    Given a sequence $v_0$, $v_1$, $\ldots$, $v_n$, the encoded sequence is $v_0,\, v_1 - v_0,\, v_2 - v_1,\, \ldots,\, v_n - v_{n-1}$.
    ...
    \item[\acf{RLE}] replaces consecutive runs of identical values with (value, count) pairs.
    ...
    It is most effective for low-cardinality, clustered columns, e.g., a road-class column sorted by class.
\end{description}
...
A physical-level technique packs a value sequence (the possible output of a logical chain) into bytes, and each integer column applies to exactly one.
Our encoder uses two:

\begin{description}
    \item[\acf{VarInt}] represents each integer with a variable number of bytes, using 7~bits per byte for data and 1~bit as a continuation flag, so values in $[0, 127]$ take 1~byte, $[128, 16383]$ take 2~bytes, and so on\footnote{\url{https://protobuf.dev/}, last accessed 2026-04-21}.
    VarInt expects unsigned values, which is why a signed integer column is normally preceded by a zigzag remap.

    \item[\acf{FastPFOR}] partitions the input into fixed-size blocks (typically 128 or 256 values), computes the minimum bit width that fits most values in each block, and packs at that width, with outliers in a separate patch list~\cite{lemireDecodingBillionsIntegers2015}.
...
A third school sidesteps both rule and model and decides empirically.
BtrBlocks~\cite{kuschewskiBtrBlocksEfficientColumnar2023} trial-encodes candidate schemes on a sample of each column block and keeps the smallest result, arguing that lightweight on-sample trials are both more accurate than a cost model and cheap enough to run at encode time.
...
It stores each unique string once and replaces occurrences with integer indices, which is effective when many features share the same value (e.g., thousands of features with \mlval{class} = \mlval{residential}).
...
Unlike general-purpose compressors such as LZ77~\cite{zivUniversalAlgorithmSequential1977} or zstd~\cite{colletZstandardCompressionApplication2021}, \ac{FSST} preserves random access, so each compressed string can be decompressed independently.
The symbol table is stored once and amortized over all strings in the column.
...
In tile encoding, such curves serve as a logical encoding that maps a 2D coordinate to a single integer column (\cref{sub:logical_integer_encodings}), and as a \emph{feature ordering} that sorts features before per-column encoding so that spatially adjacent features end up contiguous, improving the effectiveness of delta and \ac{RLE} on geometry and property columns.
...
The Hilbert curve has also been used as the basis for spatial index structures.
The Hilbert R-tree~\cite{kamelHilbertRtreeImproved1994} orders rectangles along the Hilbert curve to achieve tighter node packing than conventional R-trees, and FlatGeobuf\footnote{\url{http://flatgeobuf.org/}, last accessed 2026-05-11} uses a packed Hilbert R-tree for spatial indexing of its features.
...
\end{equation}
Constructing a MinHash signature requires examining every element of $A$ and $B$, so the construction time remains $\mathcal{O}(|A|+|B|)$ and the input sets must be retained throughout construction.
MinHash instead yields a constant-factor reduction in the persistent representation.
...
Benchmarking is the systematic measurement of system performance under controlled workloads and standardized metrics, used to evaluate system behavior, compare alternative implementations, and quantify the effects of optimizations.
...
The bootstrap itself, a resampling method for constructing confidence intervals without distributional assumptions, was formalized by Efron and Tibshirani~\cite{efronIntroductionBootstrap1998} and is the specific statistical technique we apply throughout our evaluation (\cref{ch:experiments}).
...
\begin{description}
    \item[Resource utilization,] such as \ac{CPU} load, \ac{GPU} utilization, and memory consumption to have pointers where further optimizations is possible.
    \item[Latency and responsiveness,] measuring the time between initiating an operation (e.g., zooming) and the first visible result.
    \item[Throughput,] typically measured in \acp{FPS} or frame time, describing how many frames can be rendered per second.
    \item[Frame stability,] which captures variations in frame time and identifies dropped frames or rendering stutters.
    \item[Network utilization,] including transferred bytes, tile request rates, and bandwidth consumption.
    \item[Power consumption] in Watts, which is relevant for mobile devices and battery-powered systems.
We do not measure power consumption directly, since our benchmark host is a desktop workstation rather than a battery-constrained mobile device.
We instead rely on transfer-size reductions and reduced draw-call counts as established proxies for energy savings on mobile targets, given that cellular radio energy is proportional to data volume and \ac{GPU} active time scales with draw-call count.
\end{description}

These can be benchmarked at different workload levels~\cite{jainArtComputerSystems1991}:

\begin{description}
    \item[Micro-benchmarks] isolate a single operation (e.g.,\ decoding a single tile, evaluating a single filter expression) and measure its cost in isolation.
They are useful for identifying bottlenecks but may not reflect real-world performance due to caching and concurrency effects.
    \item[Macro-benchmarks] measure the performance of a complete subsystem (e.g.,\ loading a full style and rendering a fixed camera path) under controlled conditions.
They capture interactions between components but require careful setup to ensure reproducibility.
    \item[Application-level benchmarks] measure end-to-end user-perceived performance (e.g.,\ time from page load to interactive map) in a realistic deployment.
...
\noindent We primarily use macro-benchmarks.
We load a full style, fetch and decode tiles, and exercise the renderer with a scripted camera animation.
Our benchmark harness controls for network variability (via a caching proxy) and \ac{GPU} variability (via hardware-accelerated rendering with ANGLE/Vulkan) to achieve reproducible results across runs.
...
The following chapter formalizes the joint optimization problem (\cref{sec:problem_statement,sec:scope}) and turns these foundations into concrete passes (\cref{sec:approach,sec:building_optimizations}) and an evaluation methodology (\cref{sec:gathering_sample,sec:evaluation}).
\end{chaptersummary}
