...
As shown in \cref{fig:basic-arch}, displaying map content at this lower level requires
    ...
    \emph{tiles} that provide the geometry (points, lines, polygons) and associated attributes (for example, building heights),
...
This requires including a text shaping engine to the client, as neither native nor web targets have for example Harfbuzz\footnote{\url{https://github.com/harfbuzz/harfbuzz}, last accessed 2026-05-26} exposed as a feature.
...
\begin{description}
    ...
    \item[Latency and responsiveness,] measuring the time between initiating an operation (for example zooming) and the first visible result.
