...
Mapbox developed \texttt{vtshaver}\footnote{\url{https://github.com/mapbox/vtshaver}, last accessed 2025-10-20}, a C++/Node.js tool that takes a vector tile and a Mapbox GL style, analyzes which layers, features, and properties the style references, and strips everything else.
...
Mapbox also offers \emph{style-optimized vector tiles}\footnote{\url{https://docs.mapbox.com/help/glossary/style-optimized-vector-tiles}, last accessed 26.5.26} as a proprietary server-side feature.
When a tile request includes style metadata, the server analyzes the style's sources, filters, and zoom ranges, then strips all data not visible for that style.
...
The \ac{OMT} schema~\cite{maronVectorTilesAre2025} defines which attributes are included at which zoom levels for OpenStreetMap data.
...
Columnar storage systems built on Parquet (whose nested representation originates in Dremel's record shredding and assembly~\cite{melnikDremelInteractiveAnalysis2010}) or ORC depend on this pushdown, and Abadi et~al.~\cite{abadiMaterializationStrategiesColumnOriented2007} characterized the early versus late materialization trade-off it implies.
Our data level optimizations are explained in \cref{sub:data_optimizations}.

\section{Tile Encoding and Format Innovation}\label{sec:rw_tile_encoding}

In industry, the dominant open wire format for vector tiles is \ac{MVT}\footnote{\url{https://github.com/mapbox/vector-tile-spec/tree/master/2.1}, last accessed 2026-04-21}, which uses Protocol Buffers\footnote{\url{https://protobuf.dev/}, last accessed 2026-04-21} to encode features in a row-oriented layout.
\ac{MVT} has been adopted as a conformance class in the \ac{OGC} API-Tiles standard~\cite{masoOGCAPITiles2022}, making it the baseline format for any standards-compliant tile delivery.
...
The encoding techniques used in \ac{MLT} draw on the columnar database compression literature.
Lemire and Boytsov~\cite{lemireDecodingBillionsIntegers2015} introduced \ac{SIMD}-vectorized integer compression \ac{FastPFOR} achieving billions of integers per second in decompression throughput.
Boncz et al.~\cite{bonczFSSTFastRandom2020} developed \acf{FSST}, a lightweight string compression scheme using a static symbol table that supports random access without decompressing surrounding data.
...
This competition is an instance of \emph{instance-optimized} system design as proposed by Kraska~\cite{dingSageDBInstanceOptimizedData2022}.
Rather than committing to a fixed encoding plan, the encoder uses the data distribution of each tile to select optimal physical representations.
That is the approach advocated by the SageDB vision~\cite{dingSageDBInstanceOptimizedData2022} and realized concretely by Ding et~al.~\cite{dingInstanceOptimizedDataLayouts2021}, who showed that instance-optimized data layouts reduce blocks accessed by 93\% on cloud analytics workloads, and at the column-block level by BtrBlocks~\cite{kuschewskiBtrBlocksEfficientColumnar2023}.
...
Parquet's per-column-chunk metadata is amortized over the large row groups of analytical workloads, but at the \qtyrange{10}{100}{\kilo\byte} tile sizes we care about that overhead dominates the payload.
...
Our encoding level optimizations are explained in \cref{sub:encoding_strategies}.

\section{Style Optimization}\label{sec:rw_style_optimizations}

In contrast to data-level optimizations, few prior works address style-level optimization.
...
The result is that production styles routinely contain default-valued properties, unreachable filter branches, and duplicated paint blocks.
...
Hague et al.~\cite{hagueCSSMinificationConstraint2019} formalize \ac{CSS} rule merging as a Max-SAT problem over a node-weighted bipartite graph of rules and properties with ordering constraints from the cascade, and their SatCSS tool beats six production \ac{CSS} minifiers on real benchmarks.
...
Our style level optimizations are explained in \cref{sub:expression_optimizations,sub:structural_optimizations}.
...
Style-side work is scarce.
Stamen's default-stripping utility and a few validators cover individual optimizations, but no published tool composes multiple passes, and the closest academic analogue (\ac{CSS} minification) lives in a different domain altogether.
...
Our contribution is the joint $(S, T) \to (S', T')$ optimization formalized in \cref{sec:problem_statement}, together with a cumulative ablation that distinguishes additive from synergistic composition.
\end{chaptersummary}
