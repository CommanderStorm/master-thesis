...
Per the \ac{TRS} framing of \cref{sub:fixpoint}, the expression-level phase applies the entire rewrite rule set until the style \ac{JSON} stops changing, the standard normal-form criterion.
...
Geometry-type mismatch is particularly effective on schemas such as \ac{OMT}, where a single source layer (e.g.\ \texttt{transportation}) carries mixed geometry. \mlval{fill} layers require polygons, \mlval{circle} requires points, \mlval{line} draws on polygons or lines but not points, and \mlval{symbol} layers draw on all three and so are never killed by this rule.
...
\begin{description}
	\item[\textbf{Property usage.}] Each property name appearing in a \texttt{[\mlop{"get"},~\ldots]} or \texttt{[\mlop{"has"},~\ldots]} expression in any live style layer, the zoom range over which it is referenced.
	Properties not referenced at any zoom level are candidates for removal.
	
	\item[\textbf{Geometry usage.}] Geometry types (Point, LineString, and
	Polygon) required at each zoom level, inferred from the live style layers
	targeting the source-layer.
	
	\item[\textbf{Zoom-level coverage.}] Zoom levels for which at least one live
	style layer references the source-layer.
	
	\item[\textbf{Categorical-value usage.}] Property values referenced by
	equality filters and match expressions, together with their applicable zoom
	ranges.
	
	\item[\textbf{Feature-ID usage.}] Whether any live style layer references
	\texttt{[\mlop{"id"}]} or
...
\begin{description}
    \item[\textbf{Plain}] means raw bytes with length prefixes.
    The length stream is itself subject to the integer encoding competition.
    \item[\textbf{Dictionary}] is a deduplicated corpus with per-string offset indices.
    Effective when many features share the same string value (e.g., road names that repeat across tiles).
    \item[\textbf{\ac{FSST}} (\cref{sub:string_compression})]~\cite{bonczFSSTFastRandom2020} is a lightweight, random-access string compressor that learns a symbol table of up to~255 frequent byte sequences and replaces occurrences with single-byte codes.
    ...
    \item[\textbf{\ac{FSST} + Dictionary}] combines both deduplication and byte-level compression.
\end{description}
...
\begin{description}
    \item[\textbf{Feature count}] total number of features across all tiles\footnote{Contrary to naive reasoning, features appearing in multiple tiles at different zoom levels are counted multiple times, as the optimizer reasons per-tile, not per-geographic-entity.
...
Three of these styles -- ICGC~Fosc, ICGC~Gris, and Americana -- use legacy \mlprop{\texttt{text-field}} syntax that an upstream bug in MapLibre's bundled style migrator (\texttt{gl-style-migrate}) miscompiles. Layer merging (step~15) then builds on the wrong intermediate, so any later step would compare apples vs.\ oranges against the rest of the corpus. We therefore cap these three at step~14 in the cumulative ablation. Step~17 (tile shave only, applied to the baseline) is still reported for Americana, but not for the two ICGC styles whose referenced Catalonia tile data is not available to us. The two BasemapDE styles use a different schema and were not benchmarked within the project's time budget, so the deep corpus carries 13 benchmarked styles of the 15 curated above. See the corpus-exclusions note at the head of \cref{ch:experiments} for the per-style consequences.
...
\begin{description}
    ...
    \item[\textbf{Time to first tile}] time from map initialisation until the first tile is decoded and ready for rendering.
    \item[\textbf{Time to first frame}] time until the first frame is composited and presented.
    \item[\textbf{\ac{FPS}}] average rendering throughput during the animation phase.
    \item[\textbf{Frame time percentiles}] distribution of per-frame render times, capturing both typical and worst-case latency.
    \item[\textbf{Jank count}] number of frames whose render time exceeds twice the median, indicating visible stuttering relative to typical frame pacing.
