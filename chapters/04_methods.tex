...
\begin{description}
    ...
    \item[\ac{FSST} (\cref{sub:string_compression})]~\cite{bonczFSSTFastRandom2020} is a lightweight, random-access string compressor that learns a symbol table of up to~255 frequent byte sequences and replaces occurrences with single-byte codes.
...
We curated a corpus of 15 publicly available styles that use the \ac{OMT} or BasemapDE schema:
...
The two BasemapDE styles use a different schema and were not benchmarked within the project's time budget, so the deep corpus carries 13 benchmarked styles of the 15 curated above.
...
We measure the pipeline with a \textbf{19-step cumulative ablation} (plus one plain-tile-shaving comparison configuration) across \textbf{15~styles, 18~stratified scenarios}, and 15~repeat runs per cell on the Germany \ac{OMT} tileset, plus a 13-style generalization corpus and a preprocessing-cost model that decomposes the offline cost into four additive stages.
