...
The preceding chapters established that no existing tool jointly optimizes map styles and tile data.
This chapter applies the compiler, database, and encoding analogues of \cref{ch:theoretical_background} in a concrete Rust optimizer that consumes a MapLibre style~\ac{JSON} document with optional tile statistics and emits a visually equivalent style with optimized tile data.

\section{Formalized Problem Statement}\label{sec:problem_statement}
...
Let $\mathcal{T}$ be the set of tile sets, each a finite family of tiles indexed by source and $(x, y, z) \in \mathbb{N}_+^3$.\newline
Let $\mathcal{V}$ be the set of \emph{viewing states}: tuples $(\text{lat}_\text{center}, \text{lat}_\text{center}, z, \text{roll}, \text{pitch}, \text{bearing}, \text{width}_\text{viewport}, \text{height}_\text{viewport}) \in \mathbb{R}^8$. The six degrees of freedom plus the viewport.\newline
Let $\mathcal{I}$ be the set of rasterized images and let $\bot \notin \mathcal{I}$ denote a runtime evaluation error that aborts rendering, as introduced for ill-formed feature-state and global-state references in \cref{sub:feature_state}.
...
The deployment cost is intentionally not formalized as a single closed-form function.
It depends on hardware, camera scenario, and tile cache state, and is therefore measured empirically rather than declared analytically.
$\mathcal{O}$ is correspondingly not posed as a single-objective minimization.
Several passes (e.g.,\ selectivity reordering, \cref{sub:expression_optimizations}) deliberately trade a small regression in load time for a larger improvement in FPS, and the encoder competition (\cref{sub:encoding_strategies}) locally minimizes encoded byte size as a contributing quantity to load time without committing to any weighting between the deployment axes.
We characterize each pass empirically against the metric battery of \cref{sub:metrics} in the ablation of \cref{ch:experiments}, and the resulting per-axis breakdown is what lets an operator compose the pass set that suits the deployment priorities.
...
An optimization pass may suppress a runtime evaluation error that the original style would have raised, but it must never introduce one that the original would not have raised.
...
Within MapLibre, the web renderer \texttt{maplibre-gl-js} is the reference implementation that first tracks new specification features and exposes the richest instrumentation hooks for Puppeteer\footnote{\url{https://pptr.dev/}, last accessed 2026-05-23}-driven benchmarking, which is what makes it our benchmark target rather than the Native or mobile renderers that share the same styling engine.
...
We report no \ac{RAPL} or battery deltas because the benchmark host is a desktop workstation rather than a mobile device (\cref{sub:metrics}).
    ...
    Writing a visually non-lossy geographic simplification is deferred to future work.
    We therefore quantify the size reductions achievable \emph{without} touching geometry, noting that there is headroom that composition with a geographic simplifier could reach at the cost of changing the style output.
...
No integration with a live tile server is assumed, and the optimized artifacts can be served by any standards-compliant tile infrastructure.
...
To evaluate the effectiveness of the optimizations strategies developed in this work, we adopt a benchmarking-driven approach.
We first establish a reliable performance baseline and then run cumulative ablation experiments that isolate the contribution of each optimizations pass.
...
At the center sits the \emph{optimizer}, a pipeline of passes at three levels of abstraction.
Around the optimizer are three supporting components, each detailed in its own section.
...
A validation harness exercises both pipelines against the visual-equivalence guarantee of \cref{eq:visual_equivalence} by comparing optimized and unoptimized renderings of a curated style corpus pixel-for-pixel with pixelmatch\footnote{\url{https://github.com/mapbox/pixelmatch}, last accessed 2026-05-23} against MapLibre GL~JS's Node-side software rasterizer at \texttt{pixelRatio}~1, using the upstream render-harness defaults of \texttt{allowed}~$=0.00025$ and YIQ-perceptual \texttt{threshold}~$=0.1285$.
The three pass levels at the center are:
...
\begin{enumerate}
    ...
    \item \textbf{Structural passes} (\cref{sub:structural_optimizations}) deserialize the style into the typed spec produced by the codegen front end and apply transformations that require cross-layer or cross-source reasoning, namely dead layer elimination, metadata refinement, layer merging, and cleanup.
...
We designed structural passes to operate on Rust types \emph{generated} from the upstream MapLibre GL~JS style specification rather than raw \ac{JSON}.
This shifts spec drift from a manual synchronization burden to a regenerator invocation, and gives every pass typed access to defaults, value types, and expression capabilities.
Hand-written Rust types are not a viable alternative: MapLibre publishes the specification in its own \ac{JSON}-based \ac{DSL}, and a hand-maintained shadow would require manual synchronization with every specification change and risk silent divergence.
...
This is the boundary shape a columnar encoder requires.
Each tile is handed over as a batch of parallel \texttt{MemorySegment}s rather than one call per feature, amortizing the per-boundary cost over thousands of features in one encoder invocation.

\section{Building Optimizations}\label{sec:building_optimizations}

The three pass families introduced in \cref{sub:pipeline} are detailed in turn below.
Each pass carries its own soundness argument that discharges the visual-equivalence constraint of \cref{eq:visual_equivalence}, and we state these alongside the passes rather than recapping them centrally.
The classification by data dependency (none, sampling, or full scan) is given in the table that opens each subsection.

\subsection{Expression-Level Optimizations}\label{sub:expression_optimizations}

Expression passes act on the raw \ac{JSON} representation introduced in \cref{sub:pipeline}.
\Cref{tab:expression_passes} lists them.
...
Stats-driven constant folding & scan & Fold \mlop{has}/ \mlop{get}/ \mlop{geometry-type} expressions using tile statistics; prune unreachable \mlop{match}/ \mlop{in} arms; fold comparisons against known value ranges [sparse conditional constant propagation] \\
...
Color minification           & none     & Compress color literals to their shortest representation [literal compression] \\
...
\end{tblr}
\caption{
Expression-level optimization passes, each annotated with its data dependency (\emph{none} does not have a requmrent; \emph{sampling} requires a partial tile-statistics scan; \emph{scan} requires a tile-statistics scan) and its compiler-optimization analogue.  Passes requiring no statistics run unconditionally.
...
Boolean algebra rules evaluate \mlop{all}/ \mlop{any} nodes with known operands.
A vacuous \texttt{[\mlop{"all"}]} rewrites to \mlval{true} (vacuous truth), a vacuous \texttt{[\mlop{"any"}]} to \mlval{false} (vacuous disjunction), and boolean absorption removes known-true operands from \mlop{all} and known-false operands from \mlop{any}.
...
\begin{description}
    \item[Property presence folding] if a property is present in every feature, \texttt{[\mlop{"has"},~\mlval{"p"}]} folds to \mlval{true}.
If present in none, it folds to \mlval{false}.
    \item[Single-value folding] if a property has cardinality~1 and is present in all features, \texttt{[\mlop{"get"},~\mlval{"p"}]} folds to the single known value.
    \item[Geometry-type folding] if a source layer contains only one geometry type, comparisons against \mlop{geometry-type} fold to a boolean.
    \item[Range folding] numeric comparisons against a property with known data range $[\mathrm{min},\, \mathrm{max}]$ are folded when the predicate is trivially satisfied or violated.
    ...
    We apply the equality and disequality rules only to integer-typed columns, because floating-point equality is not a reliable oracle.
    A stats min/max pair derived from observed doubles may legitimately exclude a literal by a trailing-bit margin, but the filter is still defined (and, for correctness, treated as non-foldable).
...
Stats collection uses the observed extrema rather than synthetic bounds, so the boundary comparison coincides with a value that actually occurs, which is the only case where the fold is unconditionally sound.
    \item[Match/in arm pruning] the pass removes arms referencing values absent from the tile statistics.
If all arms are pruned, the expression collapses to its fallback.
    \item[Match arm reordering] the pass reorders arms by descending frequency so the renderer evaluates the most common case first.
    \item[Data-ramp pruning] for \mlop{step} and \mlop{interpolate} expressions driven by a numeric property with known data range $[\mathrm{min},\, \mathrm{max}]$, the pass prunes stops outside the observed range.
...
\noindent A cardinality threshold of \texttt{200} governs the granularity of value tracking.
When the number of distinct values exceeds this threshold, the statistics switch from full value counts to cardinality-only mode.
...
We motivate the choice of 200 with the \ac{OMT} schema.
Every categorical property used as a \mlop{match}/ \mlop{case} discriminator in the benchmark styles (\mlval{class}, \mlval{subclass}, \mlval{admin\_level}, \mlval{kind}, land-use codes) has fewer than 200 distinct values across the Germany tileset, whereas high-cardinality string columns (feature names, identifiers) exceed it by one or more orders of magnitude.
...
We derive the defaults from the code-generated types, where each property's default is known from the upstream \texttt{v8.json}.
The pass strips only plain scalar values.
It never removes expression-valued properties, even if they would evaluate to the default at all zoom levels, to avoid the risk of changing renderer behavior when the expression's mere presence affects layout (e.g.,\ \mlprop{text-radial-offset} suppresses \mlprop{text-offset} regardless of its value).

\paragraph{Selectivity Reordering}\label{sec:selectivity_reordering}
Selectivity reordering runs as a separate pass after the peephole rewrite table has stabilized.
...
\begin{equation}\label{eq:selectivity}
    ...
    \end{aligned}
\end{equation}
We estimate compound predicates under the standard independence assumption from \cref{eq:selectivity_theory}.
...
What matters is the \emph{ordering} of predicates by selectivity, not their absolute values.
Correlation between map-style predicates (e.g.,\ \mlval{class} and \mlval{subclass} in \ac{OMT}) distorts the absolute selectivities but rarely inverts their relative ordering, so the cheap estimator suffices.
Because the rendering benefit of predicate reordering depends on the proportion of evaluation cost attributable to predicate ordering (expected to be small for styles with few/short filters), we include this pass as an exploratory extension and do not separately benchmark its isolated impact.

\paragraph{Termination and Confluence of the Fixpoint}
Per the \ac{TRS} framing of \cref{sub:fixpoint}, the expression-level phase applies to the entire rewrite rule set until the style \ac{JSON} stops changing, the standard normal-form criterion.
We imposed a hard cap of \texttt{8} iterations as a safety bound.
...
In both cases, subsequent rules within the same or next iteration typically compensate.
Boolean flattening reduces nesting depth, and constant folding absorbs the identity elements, so the net effect across iterations is a decrease in $\mu$.
...
Visual equivalence (the soundness criterion of~\cref{eq:visual_equivalence}) is preserved regardless, because every individual rule preserves expression semantics.



\paragraph{Concrete Examples}
To illustrate how expression passes compose, \cref{fig:expr_before_after} shows five representative transformations.
...
"#888"\end{lstlisting}
		...
		\caption{
Interpolation ramp collapsing and constant folding.}
		\label{fig:expr_e}
	\end{subfigure}

	\caption{
Examples of expression-level transformations applied to representative style inputs.
Each panel shows a before/after \ac{JSON} example pair demonstrating how these optimizations interact.}
...
Structural passes consume the typed style model of \cref{sub:pipeline} to perform cross-layer and cross-source reasoning.
\Cref{tab:structural_passes} lists them.
...
Dead layer/source elimination & none / sampling / scan & Remove layers with always-false filters, geometry-type mismatches (with stats), or empty source-layers; prune unreferenced sources [dead code elimination] \\
Metadata refinement          & none / sampling / scan & Extract \mlprop{minzoom}/ \mlprop{maxzoom} bounds from filter predicates and paint ramps; tighten bounds using tile coverage statistics; remove consumed zoom predicates from filters [range analysis] \\
...
Layer merging                & none      & Merge identical adjacent layers by synthesizing \mlop{case}/ \mlop{match} expressions, with zoom-tolerant matching and sort-key preservation [basic block merging] \\
...
\end{tblr}
\caption{
Structural optimization passes operating on the typed style representation, each annotated with its data dependency and compiler-optimization analogue.  Passes marked \emph{none/ sampling/ scan} run unconditionally but gain additional precision when tile statistics are available.}
...
Geometry-type mismatch is particularly effective on schemas such as \ac{OMT}, where a single source layer (e.g.,\ \texttt{transportation}) carries mixed geometry. \mlval{fill} layers require polygons, \mlval{circle} requires points, \mlval{line} draws on polygons or lines but not points, and \mlval{symbol} layers draw on all three and hence are never killed by this rule.
...
\emph{Stats-driven refinement} tightens bounds using actual tile coverage.
If a source layer's first tile appears at zoom~4, the pass raises the layer's \mlprop{minzoom} to~4 regardless of what the filter permits, since there is no data to render below that zoom.
This refinement only tightens bounds.
It never loosens them due to \cref{eq:visual_equivalence}.

Per the overzoom analysis of \cref{sec:tiles}, the refinement pass enforces two asymmetric rules to satisfy \cref{eq:visual_equivalence}:
\begin{enumerate}
    \item \textbf{\mlprop{minzoom} is symmetric.}
    Vector tiles are not \emph{underzoomed} (the renderer will not synthesize a coarser tile from a finer one) so raising \mlprop{minzoom} to the first data-carrying zoom never removes a rendered pixel.
...
\end{enumerate}
The zoom-folding rule that collapses zoom comparisons in filters is similarly one-sided.
Only \mlprop{minzoom} is used as a lower bound for the fold, because a layer's effective \mlprop{maxzoom} is not an upper bound on the camera zoom once overzooming is considered.
The source-zoom-tightening pass follows the same principle.
It tightens a source's \mlprop{maxzoom} only as far as the maximum \mlprop{maxzoom} of any layer referencing it, which already respects the overzoom rule above, and it never tightens below the source's declared bounds.
...
The motivation is that fewer layers mean fewer rendering passes.
...
\Cref{lst:painter_loop} shows the two render-pass loops the painter runs over \texttt{layerIds}: the opaque pass walks them top-to-bottom (\texttt{currentLayer} from $|layerIds| - 1$ down to \texttt{0}), the translucent pass walks them bottom-to-top, and each iteration of either loop dispatches one \texttt{this.renderLayer(...)} call.
...
Camera-only properties (such as \mlprop{line-dasharray} or \mlprop{*-translate}) must be uniform across all layers in a merge group, since they cannot be conditioned on feature attributes.
Layer types that support sort keys (\mlval{fill}, \mlval{line}, \mlval{circle}, \mlval{fill-extrusion}) are candidates.
The \mlval{symbol} type is excluded for the placement-scope reason given above.
...
For layer types that expose \texttt{\{\mlprop{fill,line,circle}\}\mlprop{-sort-key}}, the pass synthesizes a \mlop{case}/\mlop{match} sort key mapping each original layer's features to an ascending integer in declaration order, which the MapLibre renderer then applies before drawing.
The pass refuses to merge any candidate that already carries an explicit sort key, since combining a user-declared key with a synthesized one would over-determine the ordering.
\mlprop{fill-extrusion} layers are eligible in principle but inherit the same sort-key restriction, and their relative ordering is in any case dominated by the depth buffer rather than the painter's algorithm, so the synthesized key only breaks ties at co-planar surfaces.

\textbf{Layer scope.}
Any MapLibre runtime behavior whose scope is the individual layer (per-layer symbol placement against the tile-wide \texttt{CollisionIndex}, \mlop{feature-state}, \mlprop{*-translate}, and other camera-only properties) must not have that scope widened by the merge.
...
\Cref{fig:layer_merge_example} works through a minimal instance.
Two adjacent \mlval{line} layers keyed on \mlval{class} collapse into a single layer whose filter, differing paint property, and synthesized \mlprop{line-sort-key} are all \mlop{match} expressions over the same discriminator, with Phase~\stepcirc{1} (mergeability) and Phase~\stepcirc{3} (identical zoom ranges) discharged trivially.
...
The synthesized \emph{sort key} that preserves inter-layer draw order uses the discriminator value as its index under a \mlop{match} pattern and tests filters in reverse order (last painter wins) under a \mlop{case} pattern.
The pass emits a sort key only for layer types that support \mlprop{*-sort-key} and that do not already define one.
...
\begin{figure}[H]
    ...
    \end{tikzpicture}
    \caption{
Layer merging example.  Two adjacent line layers differing only in filter literal and color are merged via the \mlop{match} pattern (Phase \stepcirc{2}).
    The merged layer uses \texttt{[\mlop{"match"}, [\mlop{"get"}, \mlval{"class"}], \ldots]} for the filter, each differing paint property, and a synthesized sort key that preserves the original draw order.
    Phase \stepcirc{1} validates that all differing properties support feature-driven expressions, and Phase \stepcirc{3} confirms identical zoom ranges, so no zoom guards are needed}
    \label{fig:layer_merge_example}
\end{figure}

\subsection{Data-Level Optimizations}\label{sub:data_optimizations}

Data-level passes act on the served tile payload introduced in \cref{sub:pipeline}, drawing on the encoding background surveyed in \cref{sec:data_encoding}.
\Cref{tab:data_passes} lists them.
...
Tile shaving eliminates unreferenced data, re-encoding transforms the wire format, and compression optimizations select the most compact per-column encoding strategy, forming a logical, physical, and cost pipeline.}
...
\begin{description}
    \item[Style-driven tile shaving] (\cref{sub:tile_shaving}): the pipeline analyzes the optimized style to produce a \emph{tile pruning advisory} that specifies, per source-layer and zoom level, which properties, geometry types, and property values to retain.
    It then prunes tiles according to this advisory before encoding.
    \item[Data transformations] (\cref{sub:string_property_interning,sub:feature_reordering}): the pipeline transforms remaining data to improve compressibility, replacing categorical string properties by frequency-ordered integers (\emph{string interning}) and reordering features to exploit spatial or categorical locality.
    \item[Encoding strategy selection] (\cref{sub:encoding_strategies}): the pipeline re-encodes the pruned, transformed data into \ac{MLT} using a multi-strategy competition that automatically profiles each data stream and selects the most compact encoding.
\end{description}
...
\noindent The advisory captures five categories of information for each source-layer:

\begin{description}
	\item[Property usage.] Each property name appearing in a \texttt{[\mlop{"get"},~\ldots]} or \texttt{[\mlop{"has"},~\ldots]} expression in any live style layer, the zoom range over which it is referenced.
	Properties not referenced at any zoom level are candidates for removal.

	\item[Geometry usage.] Geometry types (Point, LineString, and
	Polygon) required at each zoom level, inferred from the live style layers
	targeting the source-layer.

	\item[Zoom-level coverage.] Zoom levels for which at least one live
	style layer references the source-layer.

	\item[Categorical-value usage.] Property values referenced by
	equality filters and match expressions, together with their applicable zoom
	ranges.

	\item[Feature-ID usage.] Whether any live style layer references
	\texttt{[\mlop{"id"}]} or
	\texttt{[\mlop{"feature-state"},~\ldots]}.
...
\begin{figure}[htbp]
    ...
    \begin{tikzpicture}[
        ...
        \draw[arrow] (prune) -- (mlt);

        \node[steps, below=0.5cm of prune] (ops) {%
            \hfil\textbf{Pruning operations (in order):}\hfil\\[2pt]
...
We derive the interning table from the tile statistics.
We sort all distinct values of a string property by descending frequency and assign each value a sequential integer starting from~0.
...
For example, the optimizer rewrites a filter \texttt{[\mlop{"=="}, [\mlop{"get"}, \mlval{"class"}], \mlval{"primary"}]} to \texttt{[\mlop{"=="}, [\mlop{"get"}, \mlval{"class"}], \mlval{0}]} if \mlval{"primary"} is the most frequent value.
...
The final wire format consists of three concatenated sections per layer, namely a header (layer name, tile extent, column count), a metadata section (column types, encoding descriptors), and a data section (the encoded streams).

The encoder runs a \emph{multi-strategy competition} over per-stream encodings.
...
The broader lineage of this trial-based approach is discussed in \cref{sec:data_encoding}.
...
We deliberately made the inner competition criterion \emph{single-objective} (smallest encoded size).
...
We report them separately per metric in the ablation evaluation (\cref{ch:experiments}), consistent with \cref{sec:problem_statement}.
The deployment metrics of \cref{sub:metrics} are characterized per-axis rather than aggregated into a single objective, and the encoder's local size minimization is a contributing quantity to the load-time axis rather than a global cost function.
...
Row-oriented features are converted to a staged columnar layer, fully encoded to wire-ready byte buffers by the encoder that implements the multi-strategy competition, and written to a byte sink via a final zero-copy step.}
    ...
    \end{subfigure}
    \caption{
Decoder~\subref{fig:arch_dec_sub} and ancoder~\subref{fig:arch_enc_sub} data-flow in \texttt{mlt-core}}
    \label{fig:arch_enc}
\end{figure}
...
Different orderings favor different encoding schemes.
Spatial orderings cluster nearby geometries and their correlated properties, improving \texttt{\ac{RLE}} run lengths.
...
A layer dominated by a single repeated property value may benefit most from spatial clustering, while a layer with sequential IDs and high-cardinality properties may favor ID ordering.
...
The encoder therefore tries multiple sort strategies on the same data and retains the encoding with the smallest total byte count.
\Cref{fig:sort_competition} diagrams the resulting trial-and-retain loop, including the bounding-box pruning that skips spatial trials for large, widely-spread layers.
The implemented strategies and their rationale are

\begin{description}
    \item[Unsorted.] The encoder preserves the original feature order.
    \item[Spatial (Morton/Z-order).] The encoder sorts features by the Z-order curve index of their first vertex, computed via bit-interleaving.  Fast to compute and provides reasonable spatial locality (\cref{sub:space_filling_curves}).
    \item[Spatial (Hilbert).] The encoder sorts features by the Hilbert curve index of their first vertex.  Slower to compute than Morton but achieves superior spatial locality, as the Hilbert curve avoids the long jumps inherent in the Z-order curve's recursive structure~\cite{moonAnalysisClusteringProperties2001}.
    \item[ID.] The encoder sorts features by feature ID in ascending order.
\end{description}
...
We derived the thresholds experimentally from Germany-wide \ac{OMT}.

In an inner loop, a per-stream encoding competition tries every viable integer encoding for the data stream.
Each candidate pairs a \emph{logical} encoding (\texttt{Plain}, \texttt{Delta}, \texttt{\ac{RLE}}, or \texttt{DeltaRle}) with a \emph{physical} encoding (\ac{VarInt} or \ac{FastPFOR}~\cite{lemireDecodingBillionsIntegers2015}).
...
Instead, a \emph{data profiler} samples a contiguous block taken from the middle of the stream.
It profiles streams with at most $512$ values in full, and profiles longer streams over $\lfloor |s| / 100 \rfloor$ values clamped to $[512,\, 16384]$.
We chose the contiguous mid-stream window over a random sample because permutation destroys \texttt{\ac{RLE}} run lengths and delta-bit-width statistics, and because the middle of a spatially or ID-sorted stream is a representative point away from the boundary artifacts at the ends.
The profiler completes a single pass over the sample to prune candidates before any full encoding is attempted.
It excludes \texttt{\ac{RLE}}, when the average run length $\bar{S} = |s| / n_{\text{runs}}$ falls below 2.0.
It excludes \texttt{Delta}, when the stream is non-monotonic and the summed bit width of the zigzag-encoded deltas exceeds that of the raw values.
It excludes \texttt{DeltaRle}, when the delta-stream average run length $\bar{S}_\Delta = (|s|-1) / n_{\Delta\text{-runs}}$ falls below 2.0 and neither \texttt{\ac{RLE}} nor \texttt{Delta} is independently viable.
...
String columns compete across four strategies, reflecting the two orthogonal dimensions of string compression, namely deduplication and byte-level compression.

\begin{description}
    \item[Plain] means raw bytes with length prefixes.
    The length stream is itself subject to the integer encoding competition.
    \item[Dictionary] is a deduplicated corpus with per-string offset indices.
    Effective when many features share the same string value (e.g., road names that repeat across tiles).
    \item[\ac{FSST}~\cite{bonczFSSTFastRandom2020} (\cref{sub:string_compression})] is a lightweight, random-access string compressor that learns a symbol table of up to~255 frequent byte sequences and replaces occurrences with single-byte codes.
    ...
    \item[\ac{FSST} + Dictionary] combines both deduplication and byte-level compression.
\end{description}

A viability probe ensures that the encoder only attempts \ac{FSST}, when the symbol-table overhead (up to \qty{2040}{\byte}) is likely to be recouped.
It excludes columns below \num{2048}~raw bytes and evaluates a trial compression on a sample of up to \num{256} strings before committing.
...
\begin{figure}[htbp]
	...
	\resizebox{\textwidth}{!}{\begin{tikzpicture}[
			...
			\node[stepbox] (tbl) {%
				\begin{tabular}{@{}l l@{}}
					...
					\midrule
					\mlval{name:de}  & Straßburg, Freiburg, Mühlhausen, \ldots \\
					\mlval{name:fr}  & Strasbourg, Fribourg, Muhlhouse, \ldots \\
					\mlval{class}    & motorway, trunk, primary, \ldots \\
					\mlval{subclass} & motorway, trunk, primark, \ldots \\
					\mlval{maxspeed} & 30, 50, 80, \ldots
				\end{tabular}%
			};
			...
			\node[stepbox, right=0.8cm of tbl, anchor=west] (hash) {%
				\begin{tabular}{@{}l c c@{}}
					...
					\midrule
					\mlval{name:de}  & $[h_1\!\ldots\!h_{128}]_{\text{val}}$ & $[h_1\!\ldots\!h_{128}]_{\text{3g}}$ \\
					\mlval{name:fr}  & $[h_1\!\ldots\!h_{128}]_{\text{val}}$ & $[h_1\!\ldots\!h_{128}]_{\text{3g}}$ \\
					\mlval{class}    & $[h_1\!\ldots\!h_{128}]_{\text{val}}$ & $[h_1\!\ldots\!h_{128}]_{\text{3g}}$ \\
					\mlval{subclass} & $[h_1\!\ldots\!h_{128}]_{\text{val}}$ & $[h_1\!\ldots\!h_{128}]_{\text{3g}}$ \\
					\mlval{maxspeed} & $[h_1\!\ldots\!h_{128}]_{\text{val}}$ & $[h_1\!\ldots\!h_{128}]_{\text{3g}}$ \\
				\end{tabular}%
			};
			...
			\node[stepbox, anchor=north] (sim) at ($(hash.south)+(0,-0.8cm)$) {%
				\begin{tabular}{@{}l c c l@{}}
					...
					\textbf{Observation} \\
					\midrule
					\mlval{name:de} vs.\ \mlval{name:fr}
					& 0.14 & \textbf{0.35}
					& shared trigrams\\

					\mlval{class} vs.\ \mlval{subclass}
					& 0.45 & \textbf{0.57}
					& shared values \\

					\mlval{name:de} vs.\ \mlval{class}
					& 0.00 & \textbf{0.01}
					& unrelated \\

					\mlval{maxspeed} vs.\ others
					& 0.00 & 0.00
					& no overlap \\
				\end{tabular}%
			};
			...
			\node[stepbox, cmp] at ($(tbl |- sim)$)  (res) {%
				...
				\fbox{\strut\mlval{name:de}, \mlval{name:fr}}\\[1pt]
				\quad{\scriptsize trigrams detected cross-language overlap}\\[4pt]
				\fbox{\strut\mlval{class}, \mlval{subclass}}\\[1pt]
				\quad{\scriptsize exact values detected semantic overlap}\\[4pt]
				\mlval{maxspeed} - individual dictionary\\[1pt]
	...
	\caption{
		...
		\circled{2}~Two 128-permutation MinHash sketches are computed per column, one over exact string values and one over byte trigrams (3-byte sliding windows).
		...
		Exact matching detects columns that share literal values (\mlval{class}/ \mlval{subclass}).
		Trigram matching detects cross-language morphological variants (\mlval{name:de}/ \mlval{name:fr} in a border region).
...
The \ac{MLT} format supports shared dictionaries where multiple string columns reference a common deduplicated corpus, amortizing dictionary overhead across columns with overlapping vocabularies.
The reference encoder groups columns by naming prefix (e.g., \mlval{name}, \mlval{name:en}, \mlval{name:de}), which misses semantic overlaps (e.g., \mlval{class} and \mlval{subclass} in \ac{OMT} share many values).
...
\begin{sloppypar}
    ...
    The variance bound from \cref{sub:lsh} ($\mathcal{O}(1/k)$) gives a standard error of $\approx 8.8\%$ at $k = 128$, adequate for the binary accept/reject decision of the clustering step.
...
The low threshold reflects that even small vocabulary overlap reduces the marginal cost of adding a column to an existing dictionary.
    ...
    We selected the threshold and similarity mode via a sweep over $[2.5\%, 20\%]$ on the Germany \ac{OMT} tileset under three similarity modes (\emph{hybrid} taking $\max(\hat{J}_\text{exact}, \hat{J}_\text{trigram})$, \emph{only-trigram}, \emph{only-exact}), summarized in \cref{fig:minhash_sweep} in the appendix.
...
The Morton/Hilbert-code dictionary approach computes a Z-order code for each unique vertex pair, stores the deduplicated codes as a sorted dictionary, and references them via per-vertex offset indices.
This produces two streams (offsets and dictionary deltas) instead of one.
...
Sequential IDs reach \texttt{DeltaRle}, constant IDs reach \texttt{\ac{RLE}}, and short or irregular sequences fall back to plain \ac{VarInt}.
...
\begin{description}
    \item[Feature count.] Total number of features across all tiles\footnote{
    Counterintuitively, a feature that appears in multiple tiles - including the same feature across zoom levels - is counted once per tile.
    This is intentional as our optimizer reasons per tile, not per geographic entity.
    The resulting bias is representative of the map-rendering use case, since client-side maps render tiled features rather than geographic ones.}.
    \item[Features by zoom.] A per-zoom-level feature count, used by metadata refinement to determine the first and last zoom level at which data exists.
    \item[Geometry types.] Counts of Point, LineString, and Polygon features, used by dead elimination to detect geometry-type mismatches.
    \item[Feature ID presence.] Whether any feature carries a non-zero feature ID, used by the advisory to decide whether to strip the ID column.
    \item[Per-property statistics.] For each property name, the statistics depend on the wire type:
    \begin{description}
        \item[\emph{Boolean}] Present count and true count.
        \item[\emph{Integer}] Present count, minimum, maximum, cardinality, and (if cardinality $\leq 200$) a full value-frequency map.
...
All optimizations described above run \emph{offline}, paid once per (style, tileset) build and amortized across every subsequent tile request and rendering session.
...
We report wall-clock cost on the Germany \ac{OMT} tileset in \cref{sub:preprocessing_cost}.
...
Our deep evaluation runs 13 styles (\cref{sub:deep_corpus}) against reproducible tile data (\cref{sub:tile_data}) and a stratified scenario sample (\cref{sub:scenarios}).
A further 13 styles (\cref{sub:generalization_corpus}) carry static-only checks to test whether gains generalize beyond the deep tier.

\subsection{Benchmarking Corpus}\label{sub:deep_corpus}

We curated a corpus of 13 publicly available styles that use the \ac{OMT} schema:
four \textbf{OpenFreeMap} styles (liberty, bright, positron, fiord);
seven \textbf{\ac{OMT} community} styles (dark-matter, osm-bright, klokan-basic, toner, osm-liberty, americana, stadia-outdoors);
and two \textbf{government and institutional} styles (ICGC~fosc, ICGC~gris).
...
ICGC~Fosc, ICGC~Gris, and Americana use legacy \mlprop{\texttt{text-field}} syntax that an upstream bug in MapLibre's bundled style migrator (\texttt{gl-style-migrate}) miscompiles.
...
We therefore cap these three at step~14 in the cumulative ablation.
Step~17 (tile shave only, applied to the baseline) is still reported for Americana, but not for the two ICGC styles whose referenced Catalonia tile data is not available to us.
See the corpus-exclusions note at the head of \cref{ch:experiments} for the per-style consequences.

\subsection{Generalization Corpus}\label{sub:generalization_corpus}
...
We sourced tile data from \ac{OSM} via Planetiler or the German government.
...
To ensure reproducibility across benchmark runs, we route all tile requests through a local caching proxy that stores tiles on first access and replays them on subsequent runs with the latency and download speed of a 4G internet connection according to Rusan et al.~\cite{rusanAssessmentPacketLatency2015} ($\frac{\text{size}}{30\,\mathrm{Mbit/s}} + 25\,\mathrm{ms}$).
...
The full cross-product of 15 locations and 5 animations is 75 pairs, which is too expensive\footnote{Each cell requires 15 repeat runs for statistical validity across 19 ablation steps and 13 styles, as this would result in $75 \times 13 \times 19 \times 15 = \num{277875}$ full animations} to run for every ablation step.
...
Within each stratum we selected the specific (location, animation) pair that exercises the most layers in the densest style in the corpus, ensuring that the sample is biased \emph{against} the null hypothesis of \enquote{no effect}, i.e.\ toward scenarios where any rendering-cost differences are most likely to be visible.
...
Our evaluation reports the deployment metrics framed in \cref{sec:problem_statement} along several axes, namely data savings (which feed the load-time axis), rendering performance, optimization cost (the $C_\text{total}$ breakdown of \cref{sub:preprocessing_cost_model}), and qualitative efficiency.

\begin{description}
    \item[Data savings] reduction in style size (raw, gzip, and brotli) and tile transfer volume.
    \item[Rendering performance] client-side metrics including load time, frames per second, frame time percentiles (p50, p95, p99), jank count, style parse time, time to first tile, and time to first frame.
    \item[Optimization cost] preprocessing time introduced by each pass, measured as wall-clock time on the optimizer's host machine.
    This is an offline cost amortized over all subsequent tile deliveries.
    \item[Efficiency] \emph{qualitative} discussion of the energy implications of reduced data transfer and fewer rendering operations.
    We perform no direct power or energy measurement (\ac{RAPL}, battery delta), because the benchmark harness runs on a desktop workstation rather than a thermally- and battery-constrained mobile device.
...
\begin{description}
    \item[Load time] wall-clock time from requesting the map to completion of the initial render, including style parsing, tile fetching, tile decoding, and first paint.
    ...
    \item[Style parse time] time spent parsing and compiling the style \ac{JSON} into the renderer's internal representation.
    \item[Time to first tile] time from map initialization until the first tile is decoded and ready for rendering.
    \item[Time to first frame] time until the first frame is composited and presented.
    \item[\ac{FPS}] average rendering throughput during the animation phase.
    \item[Frame time percentiles] distribution of per-frame render times, capturing both typical and worst-case latency.
    \item[Jank count] number of frames whose render time exceeds twice the median, indicating visible stuttering relative to typical frame pacing.
    \item[Heap memory] JavaScript heap usage during the benchmark, capturing the memory cost of style structures and tile data.
    \item[Idle time] time the renderer spends idle (waiting for tiles or \ac{GPU} sync), indicating headroom.
\end{description}
...
Our primary evaluation methodology is cumulative ablation.
We add passes one at a time in a fixed order and re-run the full benchmark suite after each addition.
This reveals both the marginal contribution of each pass and the interactions between passes.
The ablation comprises 19~cumulative steps (a baseline with no optimizations followed by passes added incrementally in the order listed in \crefrange{tab:expression_passes}{tab:data_passes}) plus one additional configuration that applies plain tile shaving on top of the unoptimized style, included isolating the style$\leftrightarrow$shaving interaction in \cref{sub:mlt_results}.
...
To reduce measurement noise, the benchmark harness pins rendering to real \ac{GPU} hardware via ANGLE\footnote{\url{https://github.com/google/angle}, last accessed 2026-05-23}/Vulkan\footnote{\url{https://www.vulkan.org/}, last accessed 2026-05-23} rather than the software rasterizer used for pixel-exact render tests.
...
The benchmark harness itself is a Puppeteer-driven headless browser that programmatically loads the MapLibre GL~JS renderer, applies a camera animation scenario, collects performance counters via the \texttt{Performance} \ac{API}\footnote{\url{https://developer.mozilla.org/en-US/docs/Web/API/Performance_API}, last accessed 2026-05-23} and adding custom instrumentation hooks into the renderer.

\begin{chaptersummary}
The optimizer is a \textbf{three-level pipeline} (\cref{sub:pipeline}) operating on the $(S, T) \to (S', T')$ problem of \cref{sec:problem_statement}.
Eight expression-level passes run to a fixpoint, six structural passes act on a typed style document, and data-level passes re-encode shaved tiles into \ac{MLT} via per-stream strategy competition.
The \textbf{pruning advisory} derived from the optimized style ties the levels together as a projection-pushdown plan over the tile data.
We measure the pipeline with a \textbf{19-step cumulative ablation} (plus one plain-tile-shaving comparison configuration) across \textbf{13~styles, 18~stratified scenarios}, and 15~repeat runs per cell on the Germany \ac{OMT} tileset, plus a 13-style generalization corpus and a preprocessing-cost model that decomposes the offline cost into four additive stages.

The following chapter applies this methodology to evaluate each optimization category in isolation, tests generalization beyond the benchmarking corpus, and analyzes the full pipeline's combined effect.
\end{chaptersummary}
