...
We motivated the choice of 200 with the \ac{OMT} schema.
...
We consumed both sources through the standard Web Mercator XYZ tiling scheme used by MapLibre, so all zoom-level reasoning in the thesis assumes this grid.
We accessed BasemapDE through its XYZ-compatible endpoint rather than its native \acs{WMTS}~\cite{masoOpenGISWebMap2010} grid, ensuring that the optimizer's tile indexing is uniform across the corpus.
Only for \ac{OSM} is the underlying data was fully available, so only for this source we could collect tile statistics at a sample rate of~1.0.
To ensure reproducibility across benchmark runs, we routed all tile requests through a local caching proxy that stores tiles on first access and replays them on subsequent runs with the latency and download speed of a 4G internet connection according to Rusan et al.~\cite{rusanAssessmentPacketLatency2015} ($\frac{\text{size}}{30\,\mathrm{Mbit/s}} + 25\,\mathrm{ms}$).
...
All of these would amplify the benefit of smaller tiles, so the measured load-time improvements from tile shaving were a conservative lower bound on what a real deployment would observe.
This ensures that we measured tile-fetch improvements (e.g.,\ smaller tiles due to shaving) under conditions representative of a \ac{CDN}-served deployment while remaining deterministic across runs.
