...
Each pass carries its own soundness argument that discharges the visual-equivalence constraint of \cref{eq:visual_equivalence}; we state these alongside the passes rather than recapping them centrally.
...
We motivated the choice of 200 with the OpenMapTiles schema: every categorical property that is used as a \mlop{match}/ \mlop{case} discriminator in the benchmark styles (\mlval{class}, \mlval{subclass}, \mlval{admin\_level}, \mlval{kind}, land-use codes) has fewer than 200 distinct values across the Germany tileset, whereas high-cardinality string columns (feature names, identifiers) exceed it by one or more orders of magnitude.
