...
["interpolate", ["linear"], ["zoom"],
  5,  ["match", ["literal", true], true, "#888", "#000"],
  10, "#888",
  15, "#888"
]\end{lstlisting}
		\end{minipage}
...
\Cref{fig:layer_merge_example} works through exactly this case.
Three adjacent \mlval{line} layers keyed on \mlval{class} collapse into a single layer whose filter, \mlprop{line-color}, \mlprop{line-width}, and synthesized \mlprop{line-sort-key} are all \mlop{match} expressions over the same discriminator, with Phase~\stepcirc{1} (mergeability) and Phase~\stepcirc{3} (identical zoom ranges) discharged trivially.
...
\begin{figure}[H]
    ...
    \resizebox{\linewidth}{!}{\hfuzz=50pt\begin{tikzpicture}[
        ...
        \node[above=0.15cm of l0, font=\small\bfseries] {Three adjacent line layers as input};
        ...
        \node[right=1.4cm of l1, check, text width=4.2cm] (p2) {%
            ...
            all filters access the same property \mlval{"class"} with unique literals\\
        ...
        \node[above=0.15cm of out, font=\small\bfseries] {One merged layer as output};
    ...
    \end{tikzpicture}}
    \caption{
Layer merging example.  Three adjacent line layers differing only in filter literal, color, and width are merged via the \mlop{match} pattern (Phase \stepcirc{2}).
