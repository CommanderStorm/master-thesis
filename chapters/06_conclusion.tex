...
Americana, ICGC~Fosc, and ICGC~Gris are excluded from end-to-end rendering checks by an upstream bug in MapLibre's \texttt{gl-style-migrate} that miscompiles their legacy \mlprop{text-field} syntax (\cref{sub:deep_corpus}). Of these three, only Americana is part of the shaving sweep. The ICGC pair is additionally absent because their Catalonia accessory tile data is not available to us, even though their primary \texttt{openmaptiles} source uses the OMT schema.
