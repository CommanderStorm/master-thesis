...
\begin{enumerate}[label=\textbf{C\arabic*}]
    ...
    \item The \textbf{empirical evaluation} (\cref{ch:experiments}), conducted across a 15-style deep corpus (13 benchmarked; the BasemapDE pair was excluded for data-access reasons), 18 scenarios, and 19 cumulative ablation steps.
...
All 11 \ac{OMT}-compatible styles in the deep corpus (the 10 end-to-end styles plus Americana) were measured for tile-shaving effectiveness on the Germany tileset (\qtyrange{19.8}{46.3}{\percent}, \cref{fig:tile_shave_per_style}).
Americana, ICGC~Fosc, and ICGC~Gris are excluded from end-to-end rendering checks by an upstream bug in MapLibre's \texttt{gl-style-migrate} that miscompiles their legacy \mlprop{text-field} syntax (\cref{sub:deep_corpus}); of these three, only Americana is part of the 11-style shaving sweep.
