...
Our rewrite rules (\cref{tab:all_rewrite_rules}) are individually simple, yet their composition through fixpoint iteration achieves globally significant simplification across styles of widely varying complexity (raw reductions of \qtyrange{3.2}{67.7}{\percent} across the 13-style Maputnik generalization corpus).
...
All 11 styles in the deep corpus whose referenced tile data is fully available to us (the 10 end-to-end styles plus Americana) were measured for tile-shaving effectiveness on the Germany tileset (\qtyrange{19.8}{46.3}{\percent}, \cref{fig:tile_shave_per_style}).
...
Of these three, only Americana is part of the shaving sweep.
...
The current pipeline assumes a one-shot tile rebuild, which is acceptable for static \ac{OMT} deliveries but impractical for PostGIS-backed servers whose data changes frequently.
...
The end-point of this trajectory is direct integration of the optimized \ac{MLT} decoder into MapLibre GL~JS, which would deliver the decode-throughput gains to existing deployments without requiring a separate client.
...
A simpler engine-side improvement would be to replace the encoder's exhaustive sort-strategy competition (\cref{sub:data_optimizations}) with a learned predictor in the manner of Kraska et~al.'s~\cite{kraskaCaseLearnedIndex2018} learned indexes.
