...
Shaving-only lifted the pooled median from \num{837} to \num{1688} (\qty{+101.7}{\percent} gain), style+shaving reached \num{1781} (\qty{+112.8}{\percent} gain), and the \ac{MLT} variant sat at \num{2021} (\qty{+141.4}{\percent} gain, with per-style gains of \qty{+148.1}{\percent} for Fiord and \qty{+149.9}{\percent} gain for Liberty that sat inside the \qtyrange{100}{200}{\percent} per-style range reported at the chapter end), the additional gain over plain-shaving consistent with \ac{MLT}'s lighter per-frame access cost.
...
The finding in \cref{tab:mvt_mlt_comparison} that is easiest to overlook is that the reference baseline \ac{MLT} under no compression was \emph{worse} than the commonly gzip-compressed \ac{MVT} (\qty{4.24}{\giga\byte} vs.\ \qty{3.08}{\giga\byte}).
