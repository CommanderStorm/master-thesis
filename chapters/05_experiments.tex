...
Top panel: raw, gzip, Brotli, and layer-count reductions share a single \%-reduction scale.
Bottom panel: \ac{AST} node count change is signed -- positive values denote a reduction in node count, negative values denote growth from expression rewriting (e.g.\ unrolling \texttt{match} into nested \texttt{case}); the dashed line marks zero.
