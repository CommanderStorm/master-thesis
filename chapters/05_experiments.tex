...
Values are normalised to each style's baseline so the three metrics share a single axis despite their differing absolute scales (layer counts in the tens, filter counts in the hundreds, expression-tree node counts in the thousands).
...
\begin{figure}[htbp]
	...
	\caption{Structural complexity metrics across cumulative ablation steps on the Fiord and Liberty styles, normalised to each style's baseline.
