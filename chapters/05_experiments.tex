...
The first seven passes correspond to ablation steps~1-7 of the cumulative ablation (\cref{sub:ablation}); selectivity reordering is deferred to step~16 because it requires the tile statistics of \cref{sub:tile_statistics}.
...
Beyond the rendering metrics from Experiment~1, we add three structural-complexity metrics: layer count, filter count, and total expression-node count.

\subsection{Results}\label{sub:structural_results}

\cref{fig:complexity_metrics} tracks the three structural complexity metrics introduced in \cref{sub:structural_evaluation} -- layer count, filter count, and total expression-tree node count -- across the cumulative ablation steps on the Fiord and Liberty styles.
...
Layer count drops only when layer merging fires; dead elimination rarely removes whole layers because styles are designed to expose parts of the map rather than leave layers unreferenced.
Total expression-tree node count is driven mostly by the expression passes (steps~1--7) -- Fiord falls by \qty{36.8}{\percent} (\num{1610}$\to$\num{1018}) and Liberty by \qty{10.5}{\percent} (\num{3537}$\to$\num{3167}); structural passes contribute additional, smaller reductions by removing or merging layers that own subtrees.
...
\begin{figure}[htbp]
    ...
    \begin{subfigure}[t]{\linewidth}
        ...
        All four metrics share the 0--70\,\% reduction scale; each dot is one style.}
    ...
    \begin{subfigure}[t]{\linewidth}
        ...
        On the largest style (Americana) optimization roughly halves the node count; on small styles, expression canonicalization (e.g.\ unrolling \texttt{match} into nested \texttt{case}) inflates the absolute count by a few hundred nodes -- large in \%~terms but small in absolute terms.}
...
Load-time variance compresses sharply at \texttt{tile\_shave}; the \ac{FPS} spread instead widens past the same step, exposing per-style headroom.}
...
The brief uptick at \texttt{tile\_shave\_only} corresponds to Configuration~3 of \cref{sub:mlt_evaluation} (shaving applied to the unoptimized style); the subsequent \texttt{tile\_shave} and \texttt{tile\_rewrite} steps restore the optimized style and push the curve to the pipeline's overall minimum.
...
The orange line is the mean across scenarios; thin gray lines bound the per-scenario distribution.
Most of the wire-size win lands by \texttt{strip\_metadata}; the \texttt{tile\_shave\_only} spike is the unoptimized-style baseline of Configuration~3, which \texttt{tile\_shave} and \texttt{tile\_rewrite} recover and surpass.}
