...
Across the 13 OMT-compatible benchmark styles, the seven expression passes reduce the per-style median load time by \qty{2.0}{\percent} (median across styles, range \qtyrange{-58.8}{+12.3}{\percent} per style).
...
\ac{MLT} re-encoding matches the plain-shaving load time within measurement noise (\qty{130}{\milli\second} vs.\ \qty{130}{\milli\second}) and delivers a further \ac{FPS} gain on top, despite the added decoder work at parse time.
...
To quantify this variation, we ran the full shaving pipeline (optimize with all passes, generate pruning advisory, apply advisory as \ac{MVT}) on the same Germany tileset (\qty{3.07}{\giga\byte}, \ac{OMT} schema, zoom levels 0-14) for each of the 11 \ac{OMT}-compatible styles in the benchmark corpus.
...
We benchmark ten deep-corpus styles end-to-end with tile data, eleven \ac{OMT}-compatible styles at the tile-shaving size level, and evaluate thirteen additional styles with static analysis only.
...
Values are percentage of baseline, medians across the 13 benchmarked styles and 18 scenarios (with steps~16-19 restricted to fiord and liberty per the dagger).
...
The BasemapDE pair was additionally excluded because its tile source is not \ac{OMT}-compatible and we could not provision matching shaved tiles.
For the 11 \ac{OMT}-compatible deep-corpus styles (the 10 end-to-end styles plus Americana) we verified the tile-shaving step alone (advisory generation and \ac{MVT} pruning, without rendering) on the same Germany tileset (\cref{fig:tile_shave_per_style}), confirming that shaving effectiveness generalizes across styles with reductions of \qtyrange{19.8}{46.3}{\percent}.
...
The pooled Fiord+Liberty aggregate used in the \ac{MLT} discussion (\qty{+141.4}{\percent}) is a different aggregation of the same data restricted to the two end-to-end styles, not a conflicting headline.
