...
We partly self-inflict this, because using real \ac{GPU} hardware (ANGLE/Vulkan) rather than a software rasteriser captures realistic rendering costs (\ac{GPU} buffer management, shader compilation) at the expense of slightly higher noise compared to deterministic software rendering.
...
Across every comparison, the fraction of differing pixels stayed below the configured per-test tolerance, confirming that every optimizations pass preserves visual identity on the software rasteriser within the perceptual-difference budget used by the upstream MapLibre render harness.
...
The style-only results (Experiments~1-2) cover the 13 benchmarked styles (BasemapDE Colour and BasemapDE Topo are the only deep-corpus members without measurement data) and are not affected by this limitation.
