...
Layer count drops for Liberty only when layer merging fires, while Fiord benefits from previous cleanup passes.
...
Total expression-tree node count is driven mostly by the expression passes (steps~1--7), where Fiord falls by \qty{36.8}{\percent} (\num{1610}$\to$\num{1018}) and Liberty by \qty{10.5}{\percent} (\num{3537}$\to$\num{3167}).
...
Rivian contributed the runtime performance engineering, which brings transcoding of the Germany \ac{OMT} tileset down from roughly five hours under the prior Tremmel \& Zink reference encoder to about one minute.
...
Shaving-only lifted the pooled median from \num{837} to \num{1688} (\qty{+101.7}{\percent}), style+shaving reached \num{1781} (\qty{+112.8}{\percent}), and the \ac{MLT} variant sat at \num{2021} (\qty{+141.4}{\percent}, with per-style gains of \qty{+148.1}{\percent} for Fiord and \qty{+149.9}{\percent} for Liberty that sat inside the \qtyrange{100}{200}{\percent} per-style range reported at the chapter end), the additional gain over plain-shaving consistent with \ac{MLT}'s lighter per-frame access cost.
...
\begin{figure}[H]
    ...
    \begin{subfigure}[t]{0.48\linewidth}
        \centering
        \includegraphics[width=\linewidth]{figures/rendering_metrics_mlt_fiord_loadMs.pdf}
        \caption{Fiord - load time.}
        \label{fig:rendering_metrics_mlt_fiord_load}
    \end{subfigure}\hfill
    \begin{subfigure}[t]{0.48\linewidth}
        \centering
        \includegraphics[width=\linewidth]{figures/rendering_metrics_mlt_fiord_fps.pdf}
    ...
    \begin{subfigure}[t]{0.48\linewidth}
        \centering
        \includegraphics[width=\linewidth]{figures/rendering_metrics_mlt_liberty_loadMs.pdf}
        \caption{Liberty - load time.}
        \label{fig:rendering_metrics_mlt_liberty_load}
    \end{subfigure}\hfill
    \begin{subfigure}[t]{0.48\linewidth}
        \centering
        \includegraphics[width=\linewidth]{figures/rendering_metrics_mlt_liberty_fps.pdf}
