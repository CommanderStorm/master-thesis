...
\vspace{4pt}
\textbf{Step markers used throughout this chapter.}\newline
...
The headline is the \qty{28.3}{\percent} reduction our encoder achieved over the reference baseline under no wire compression.
...
The data-driven strategy selection in our encoder reclaimed this gap and went past it: our encoder under gzip was \qty{12.3}{\percent} smaller than \ac{MVT} under gzip, and the advantage held (\qty{12.9}{\percent}) even against zstd-22, the strongest general-purpose compressor in the comparison.
...
\ac{MVT} under zstd-22 yielded \qty{3.16}{\giga\byte} for the Germany tileset, whereas our encoder under the same compressor yielded \qty{2.75}{\giga\byte}, a \qty{12.9}{\percent} reduction (\cref{tab:mvt_mlt_comparison}).
...
Step markers $\dagger$ and $\ast$: see the \hyperref[box:corpus_exclusions]{corpus-exclusions note} at the start of the chapter.
}
...
\csvreader[
  ...
  table head={\toprule
    ...
    \midrule},
  table foot={\midrule
    \multicolumn{7}{l}{\footnotesize $\dagger$ 10 end-to-end styles only (see corpus-exclusions note).\quad $\ast$ requires tile data.} \\
    \bottomrule},
  head to column names,
]{scripts/data/marginal_contribution.csv}{}%
{\step & \pass\ifnum\daggerflag=1\textsuperscript{$\dagger$}\fi\ifnum\starflag=1\textsuperscript{$\ast$}\fi &
...
Values are percentage of baseline, medians across the 13 benchmarked styles and 18 scenarios. The $\dagger$ flags steps where the aggregation drops to the 10 end-to-end styles (see the corpus-exclusions note). The $\ast$ flags steps that require tile data (statistics or tile bytes) at preprocessing time.
...
We therefore measured the overhead ratio of the full competition at $342 / 175 = 1.95\times$ in \ac{CPU} time, substantially lower than the analytical 3-5$\times$ estimate, because the cost-control mechanisms formalized in \cref{sub:preprocessing_cost_model} eliminated most redundant work.
...
With 16-thread parallelism the end-to-end wall-clock time dropped to approximately \qty{2}{\minute}, perfectly acceptable for an offline build stage.
...
Composed end-to-end across the ten deep-corpus styles, the full pipeline cut per-style median load time by \qty{71}{\percent} and lifted \ac{FPS} by \qty{152}{\percent}.
...
We ran the visual-equivalence check defined alongside \cref{eq:visual_equivalence} (with the per-pass soundness arguments collected in \cref{sec:building_optimizations}) across the 13 styles of the deep corpus, all 18 scenarios from \cref{sub:scenarios}, and zoom levels~0-14 in both axis-aligned and pitched views.
