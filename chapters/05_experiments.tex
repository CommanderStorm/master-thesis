...
Shaving-only lifted the pooled median from \num{837} to \num{1688} (a \qty{+101.7}{\percent} gain), style+shaving reached \num{1781} (a \qty{+112.8}{\percent} gain), and the \ac{MLT} variant sat at \num{2021} (a \qty{+141.4}{\percent} gain, with per-style gains of \qty{+148.1}{\percent} for Fiord and a \qty{+149.9}{\percent} gain for Liberty that sat inside the \qtyrange{100}{200}{\percent} per-style range reported at the chapter end), the additional gain over plain-shaving consistent with \ac{MLT}'s lighter per-frame access cost.
...
For multi-tenant tile servers hosting dozens of styles, operators should apply shaving per style and cache the resulting tile variants independently (the approach that Mapbox's closed-source \enquote{style-optimized vector tiles} service~\cite{StyleoptimizedVectorTiles} would be likely to use).
...
The style-parse component shrank monotonically with each expression pass, while first-tile and remaining-load were unchanged by style-only steps and dropped only once tile shaving fired at steps~17-19.
...
\begin{figure}[p]
	...
	\vspace*{-2cm}
	\makebox[\textwidth][c]{%
		\includegraphics[width=0.9\paperwidth]{figures/heatmap_loadMs.pdf}
	}
	\caption{
...
\begin{landscape}
	\begin{figure}[p]
		...
		width=\textheight,
		keepaspectratio
		]{figures/heatmap_fps.pdf}
		
		\caption{Heatmap of \ac{FPS} across scenarios (rows) and ablation steps (columns).
...
\begin{figure}[p]
	...
	\vspace*{-2cm}
	\makebox[\textwidth][c]{%
		\includegraphics[width=0.9\paperwidth]{figures/heatmap_jankCount}
	}
	\caption{
...
It is not higher than the hardware threads 8 since there is some single threaded critical section and because we are CPU bound, so we loose some of 
...
Composed end-to-end across the ten deep-corpus styles benchmarked with tile data, the full pipeline reduced load time by \qtyrange{63}{74}{\percent} (per-style median \qty{71}{\percent}) and improved \ac{FPS} by \qtyrange{100}{200}{\percent} (per-style median \qty{152}{\percent}), but the cross-family interaction was \textbf{additive rather than synergistic}.
The pooled Fiord+Liberty aggregate used in the \ac{MLT} discussion (\qty{+141.4}{\percent}) is the same data restricted to the two full-pipeline styles and pooled rather than per-style averaged, and the two figures should be read together rather than against each other.
