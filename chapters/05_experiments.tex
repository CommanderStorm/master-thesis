...
Almost all of the style-size win landed by \texttt{strip\_metadata} (step~8), and the structural and refinement passes left the curve flat because their gains were concentrated in layer count rather than serialized bytes.
The brief uptick at \texttt{tile\_shave\_only} corresponds to Configuration~3 of \cref{sub:mlt_evaluation} (shaving applied to the unoptimized style), and the subsequent \texttt{tile\_shave} and \texttt{tile\_rewrite} steps restored the optimized style and pushed the curve to the pipeline's overall minimum.

\begin{figure}[H]
    \centering
    \includegraphics[width=\linewidth]{figures/waterfall_gzip_bytes.pdf}
    \caption{
Cumulative ablation of mean style gzip bytes across all 19 steps.
The orange line is the mean across scenarios, and thin gray lines bound the per-scenario distribution.
Most of the wire-size win lands by \texttt{strip\_metadata}.
The \texttt{tile\_shave\_only} spike is the unoptimized-style baseline of Configuration~3, which \texttt{tile\_shave} and \texttt{tile\_rewrite} recover and surpass.
$\dagger$, $\ast$ see the \hyperref[box:corpus_exclusions]{corpus-exclusions note} at the start of the chapter.}
