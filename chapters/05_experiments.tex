...
Filter count is reduced primarily by metadata refinement (consuming zoom predicates) and dead elimination, with cleanup removing residual \mlval{\texttt{true}} filters that refinement leaves behind.}
...
A general-purpose schema such as OpenMapTiles exposes hundreds of properties and multiple geometry types, of which any specific style references only a fraction, so the savings achievable by tile shaving (\cref{sub:tile_shaving}) co-vary with the upstream style optimizations.
...
\begin{figure}[htbp]
	...
	\end{subfigure}
	\caption{Encoder comparison for the Germany OpenMapTiles tileset (zoom~0-14, \qty{256588}{\kilo tiles}).
...
We ran the visual-equivalence check defined alongside \cref{eq:visual_equivalence} (with the per-pass soundness arguments collected in \cref{sec:building_optimizations}) across all 15 benchmark styles, all 18 scenarios from \cref{sub:scenarios}, and zoom levels~0-14 in both axis-aligned and pitched views.
