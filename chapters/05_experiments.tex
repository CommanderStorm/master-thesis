...
Americana's tile data is fully available to us and stays in the sweep.
...
Per-style bootstrap confidence intervals showed that this aggregate effect was not consistently significant for individual styles.
...
\ac{CI} is relatively high, because parsing a style is a one time event that is not amortized across multiple frames, so the signal is more sensitive to noise than the sustained rendering time.
