...
\emph{BasemapDE Color} and \emph{BasemapDE Topo} were not benchmarked within the project's time budget due to contracting delays in getting access to the underlying data.
...
Each bar shows the median change introduced by adding one expression pass on top of all previous passes, aggregated across all 18 scenarios and the 13 benchmarked styles (the two BasemapDE styles in the deep corpus were not benchmarked, see the corpus-exclusions box at the top of this chapter).
...
\begin{figure}[htbp]
    ...
    \caption{
On-Wire data reduction from style-driven \ac{MVT} shaving across 11 \ac{OMT} styles with fully available tile data, applied to the same \qty{3.07}{\giga\byte} Germany tileset.  Styles that reference fewer source-layers and properties achieve larger reductions}
...
We benchmark ten deep-corpus styles end-to-end with tile data, eleven \ac{OMT} styles with fully available tile data at the tile-shaving size level, and evaluate thirteen additional styles with static analysis only.
...
\begin{figure}[htbp]
    ...
    \begin{subfigure}[t]{\linewidth}
        \centering
        \includegraphics[width=\linewidth]{figures/cross_style_summary.pdf}
...
For the 11 \ac{OMT} styles in the deep corpus whose referenced tile data is fully available to us, we evaluated tile shaving separately against the Germany tileset (\cref{fig:tile_shave_per_style}), confirming that data-level reductions generalize across styles.
...
Step markers $\dagger$ and $\ast$: see \cref{tab:marginal_contribution}.
}
...
We ran the visual-equivalence check defined alongside \cref{eq:visual_equivalence} (with the per-pass soundness arguments collected in \cref{sec:building_optimizations}) across the 13 benchmarked styles of the deep corpus (the BasemapDE pair was not benchmarked), all 18 scenarios from \cref{sub:scenarios}, and zoom levels~0-14 in both axis-aligned and pitched views.
...
The tile data uses only \ac{OMT} and BasemapDE schemas.
...
We benchmarked the full rendering pipeline (style optimizations through tile shaving to \ac{MLT} re-encoding) end-to-end on 10 of the 15 deep-corpus styles (bright, dark-matter, fiord, klokan-basic, liberty, osm-bright, osm-liberty, positron, stadia-outdoors, toner).
...
The style-only results (Experiments~1-2) cover the 13 benchmarked styles (BasemapDE Color and BasemapDE Topo are the only deep-corpus members without measurement data) and are not affected by this limitation.
