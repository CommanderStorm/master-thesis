...
Across the 13 benchmarked styles, the seven expression passes reduce the per-style median load time by \qty{2.0}{\percent} (median across styles, range \qtyrange{-58.8}{+12.3}{\percent} per style).
...
The single largest marginal contributor to style size is default stripping (step~6), which accounts for \qty{-4.3}{\percent} of raw style size (\qty{-2.9}{\percent} under gzip).
...
Expression passes shave only \qty{2.0}{\percent} from per-style median load time, but raw style size drops by up to \qty{20.3}{\percent} on Fiord, with most of the size win concentrated in a single pass (default stripping).
...
Only two of the style families gain significant speedups, in both cases due to typos in the style file.
...
Fiord's 48-layer style sheds two layers (\qty{-4.2}{\percent}), one from dead elimination (step~9) and one from cleanup (step~14) after metadata refinement reduces a filter on an invisible layer to \mlval{true}.
...
We credit that speedup to Rivian rather than to ourselves, and therefore do not analyse encoder performance further in this chapter.}
...
The data shows that style and data optimizations compose additively rather than synergistically.
...
Shaving-only drops the pooled median load time from \qty{396}{\milli\second} (baseline) to \qty{134}{\milli\second} (\qty{-66.1}{\percent}).
Style-only contributes a modest \qty{-2.5}{\percent}, and style+shaving reaches \qty{-67.2}{\percent}.
\ac{MLT} re-encoding matches the plain-shaving load time within measurement noise (\qty{130}{\milli\second} vs.\ \qty{130}{\milli\second}) and delivers a further \ac{FPS} gain on top.
\ac{FPS} mirrors the load-time pattern.
Shaving-only lifts the pooled median from \num{837} to \num{1688} (\qty{+101.7}{\percent}), style+shaving reaches \num{1781} (\qty{+112.8}{\percent}), and the \ac{MLT} variant sits at \num{2021} (\qty{+141.4}{\percent}, with per-style gains of \qty{+148.1}{\percent} for Fiord and \qty{+149.9}{\percent} for Liberty that sit inside the \qtyrange{100}{200}{\percent} per-style range reported at the chapter end), the additional gain over plain-shaving consistent with \ac{MLT}'s lighter per-frame access cost.
...
The finding in the table that is easiest to overlook is that the reference baseline is \emph{worse} than gzip-compressed \ac{MVT} (\qty{3.54}{\giga\byte} vs.\ \qty{3.08}{\giga\byte}).
...
\ac{MVT} under zstd-22 yields \qty{3.16}{\giga\byte} for the Germany tileset, whereas our encoder under the same compressor yields \qty{2.75}{\giga\byte}, a \qty{12.9}{\percent} reduction (\cref{tab:mvt_mlt_comparison}).
...
Each benchmark parses the tile header and materialises all features and properties, and \ac{MVT} variants include gzip decompression in the measured path.
...
Our decoder is $4.0$-$7.7\times$ faster than uncompressed \ac{MVT}.
...
However, the aggregate data shows no super-additive interaction at the median level (\cref{sub:mlt_results}).
The combined style+shaving reduction is approximately the sum of the individual reductions, not greater.
...
Tile-data reductions range from \qty{19.8}{\percent} (Bright) to \qty{46.3}{\percent} (Toner), with a median of \qty{29.0}{\percent}.
...
At the low end, Stadia Outdoors (\qty{3.2}{\percent}) and the two AWS styles (both at \qty{10.6}{\percent}) leave little for the optimizer to remove.
...
Dropping it flips the sign to $r = -0.54$, underscoring that no stable trend exists at this sample size.
...
We find no statistically significant correlation between initial style complexity and achievable reduction at this sample size ($r = 0.35$, $n = 13$, $p = 0.24$), and the relationship is dominated by a single high-leverage point.
The intuition that verbose institutional styles with several hundred layers and deeply nested expressions offer more room for improvement than simple styles with fewer than 50 layers is not borne out at this sample size.
...
The load-time interquartile range tightens incrementally across the style-only passes and contracts sharply at \texttt{tile\_shave}, where the multi-second outliers visible across the style-only stack effectively disappear.
The \ac{FPS} view mirrors this from the other direction: median \ac{FPS} roughly doubles at \texttt{tile\_shave\_only} and approximately triples by \texttt{tile\_rewrite}, with the spread widening as some styles realise more headroom than others.
...
Almost all of the style-size win lands by \texttt{strip\_metadata} (step~8), and the structural and refinement passes leave the curve flat because their gains are concentrated in layer count rather than serialized bytes.
The brief uptick at \texttt{tile\_shave\_only} corresponds to Configuration~3 of \cref{sub:mlt_evaluation} (shaving applied to the unoptimized style), and the subsequent \texttt{tile\_shave} and \texttt{tile\_rewrite} steps restore the optimized style and push the curve to the pipeline's overall minimum.
