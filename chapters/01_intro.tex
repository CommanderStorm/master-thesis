...
An interactive vector map runs two paths.
The per-tile path fetches, decodes, and binds the style to each tile's features at load time.
The per-frame path re-evaluates view-dependent expressions and submits draw calls to the \ac{GPU}.
...
They provide a client with a client-side style document driving a \ac{WebGL} or \ac{WebGPU} based rendering pipeline.
...
We ask whether such cross-level interactions produce measurably super-additive gains, or whether the levels compose merely additively, as a central empirical question of this work that we formalize in \cref{sec:problem_statement}.
...
\begin{enumerate}[label=\textbf{C\arabic*}]
    ...
    The Germany \ac{OMT} tileset provides an encoder-only, style-independent baseline.
    The ten-style \ac{OMT} deep corpus carries the end-to-end browser benchmarks.
    An eleven-style \ac{OMT} subset whose referenced tile data is fully available to us is measured for tile-shaving effectiveness only.
...
\Cref{ch:experiments} measures the isolated and combined effects through a cumulative ablation across a 15-style deep corpus (13 benchmarked) and 18~scenarios, and \cref{ch:conclusion} revisits R1-R3 in \cref{sec:rq_revisited} and names tile shaving and columnar re-encoding as the primary levers.
